...
public abstract class ExecutableCheck extends Check implements Runnable {
  ...
  protected static ScheduledThreadPoolExecutor executorService =
      new ScheduledThreadPoolExecutor(MIN_EXEC_POOL_SIZE, Core.newThreadFactory("health"));

  static {
    ScheduledThreadPoolExecutor executor = executorService;
    executor.setRemoveOnCancelPolicy(true);
    executor.setKeepAliveTime(
        Config.getInt(CHECK_DEFAULT_INTERVAL) + Config.getInt(CHECK_DEFAULT_TIMEOUT), TimeUnit.MILLISECONDS);
  }
