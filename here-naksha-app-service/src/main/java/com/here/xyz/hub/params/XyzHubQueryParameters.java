...
public class XyzHubQueryParameters extends QueryParameterList {
   ...
   */
  @Nullable
  String getString(@NotNull String key, @Nullable String alternative) throws ParameterError {
   ...
   */
  @Nullable
  Boolean getBoolean(@NotNull String key, @Nullable Boolean alternative) throws ParameterError {
    final QueryParameter queryParameter = get(key);
    if (queryParameter == null) {
      return alternative;
    }
   ...
   */
  @Nullable
  Double getDouble(@NotNull String key, double minValue, double maxValue) throws ParameterError {
    final QueryParameter queryParameter = get(key);
    if (queryParameter == null) {
      return null;
    }
    ...
    if (value < minValue || value > maxValue) {
      throw new ParameterError(
          "&" + key + " expected to be between [" + minValue + "," + maxValue + "], but was: " + value);
    }
    return value;
  }
   ...
   */
  @Nullable
  Double getWgs84Latitude(@NotNull String key) throws ParameterError {
   ...
   */
  @Nullable
  Double getWgs84Longitude(@NotNull String key) throws ParameterError {
   ...
   */
  @Nullable
  Long getLong(@NotNull String key, long minValue, long maxValue) throws ParameterError {
    final QueryParameter queryParameter = get(key);
    if (queryParameter == null) {
      return null;
    }
  ...
  public @NotNull TagsQuery getTagsQuery() {
    ...
    // Each &tags is combined via AND, within tags values are normally OR combined, except for the not URL encoded
    // plus.
    // In that case we need to split the values into two separate AND, which effectively is the same as two "&tags"
    // query parameters.
  ...
  public int getRadius() throws ParameterError {
    ...
    if (values.size() == 1) {
      ...
      if (radius == null || radius < 0L || radius > Integer.MAX_VALUE) {
        throw new ParameterError(
            "Invalid parameter " + RADIUS + "=" + values.get(0) + ", expected positive int32");
      }
    ...
    while (queryParam != null) {
      ...
      if (arguments.size() >= 1) {
        ...
        if (opName != null) {
          ...
          if (op == null) {
            throw new ParameterError(
                "Unknown operation '" + opName + "' for query parameter: " + queryParam);
          }
        } else {
  ...
  public int getVerifiedEpsg() throws ParameterError {
    ...
    throw new ParameterError("Failed to transform coordinates from " + GeoTools.WGS84_EPSG + " to EPSG:" + epsg
        + ", unsupported Geodetic Parameter Dataset");
  }
  ...
  public @NotNull Clustering getClustering() throws ParameterError {
    // TODO: We already should rewrite "&clustering.resolution=" into "&clustering:resolution=", which allows to
    // grab all values from one
  ...
  public @NotNull Tweaks getTweaks() throws ParameterError {
    // TODO: We already should rewrite "&tweak.resolution=" into "&tweak:resolution=", which allows to grab all
    // values from one
    //       key via normal parameters handling.
