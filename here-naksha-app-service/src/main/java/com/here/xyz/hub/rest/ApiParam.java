...
public class ApiParam {
  ...
  private static Object getConvertedValue(String rawValue) {
    ...
    if (rawValue.equalsIgnoreCase(".null")) return null;
  ...
  private static String getConvertedKey(String rawKey) {
    ...
    if (replacement == null && rawKey.startsWith("f.")) return rawKey.substring(2);
  ...
  public static class Query {
    ...
    public static final String IF_ROW_LOCKED = "rlock";
      ...
      for (String s : input)
        switch (s) {
          ...
          case "type":
            break;
          default:
            selection.add(s.replaceFirst("^p\\.", "properties."));
            break;
        }
    ...
    public static List<String> getSort(RoutingContext context) {
      ...
      for (String s : Query.queryParam(Query.SORT, context))
        if (s.startsWith("p.") || s.startsWith("f.")) sort.add(s.replaceFirst("^p\\.", "properties."));
        else sort.add(s);
    ...
    public static Integer[] getPart(RoutingContext context) {
      ...
      if (l.size() <= 2 && l.size() >= 1)
        try {
          ...
          total = (l.size() > 1
              ? Integer.parseUnsignedInt(l.get(1))
              : /* -1 */ 1); // -1 to use n-handle modus
          return (part == 0 || total == 0)
              ? null
              : new Integer[] {Math.min(part, total), Math.max(part, total)};
    ...
    static PropertyQuery getPropertyQuery(String query, String key, boolean multiValue) {
      if (StringUtils.isNullOrEmpty(query) || StringUtils.isNullOrEmpty(key)) return null;

      int startIndex;
      if ((startIndex = query.indexOf(key)) != -1) {
        String opValue = query.substring(startIndex + key.length()); // e.g. =eq=head
        String operation = shortOperators.stream()
            .sorted(Comparator.comparingInt(k ->
                k.length() * -1)) // reverse a sorted list because u want to get the longer ops first.
            .filter(opValue::startsWith) // e.g. in case of key=eq=val, 2 ops will be filtered in: '=eq='
            // and '='.
            .findFirst() // The reversed sort plus the findFirst makes sure the =eq= is the one you are
            // looking for.
        ...
        List<Object> values =
            multiValue ? Arrays.asList(value.split(",")) : Collections.singletonList(value.split(",")[0]);
          ...
          .forEach(keyValuePair -> {
            ...
            for (String shortOperator : shortOperators) {
              ...
              if (currentPositionOfOp != -1) {
                ...
                (!spaceProperties
                        && (op == null
                            || currentPositionOfOp < position
                            || (currentPositionOfOp == position
                                && op.length() < shortOperator.length())))
                    ||
                    // space properties query
                    (keyValuePair
                            .substring(0, currentPositionOfOp)
                            .equals(property)
                        && spaceProperties
                        && (op == null
                            || currentPositionOfOp < position
    ...
    static Map<String, Object> parseAdditionalParams(String query, String type) throws Exception {
      ...
      Stream.of(query.split("&")).filter(k -> k.startsWith(paramPrefix)).forEach(keyValuePair -> {
        try {
          keyValuePair = URLDecoder.decode(keyValuePair, "utf-8");
        } catch (UnsupportedEncodingException e) {
          e.printStackTrace();
      ...
      if (type.equals(CLUSTERING)) {
        ...
        switch (key) {
          ...
          case CLUSTERING_PARAM_RESOLUTION:
            if (!(value instanceof Long)) throw new Exception(invalidKeyMessage + "Expect Integer.");
            ...
            if (!CLUSTERING_PARAM_RESOLUTION_RELATIVE.equals(key)
                && ((long) value < 0 || (long) value > 18))
              throw new Exception(invalidKeyMessage + "Expect Integer hexbin:[0,13], quadbin:[0,18].");
            break;

          case CLUSTERING_PARAM_PROPERTY:
            if (!(value instanceof String)) throw new Exception(invalidKeyMessage + "Expect String.");
            break;
          ...
          default:
            throw new Exception("Invalid Clustering Parameter! Expect one of ["
                ...
                + CLUSTERING_PARAM_RESOLUTION_ABSOLUTE + ","
                + CLUSTERING_PARAM_PROPERTY + "," + CLUSTERING_PARAM_POINTMODE + ","
                + CLUSTERING_PARAM_COUNTMODE + "," + CLUSTERING_PARAM_SINGLECOORD + ","
          ...
          case TWEAKS_PARAM_STRENGTH:
            if (value instanceof String) {
              ...
              switch (keyS) {
                ...
                default:
                  throw new Exception(
                      "Invalid tweaks.strength value. Expect [low,lowmed,med,medhigh,high]");
              }
            } else if (value instanceof Long) {
              if ((long) value < 1 || (long) value > 100)
                throw new Exception("Invalid tweaks.strength value. Expect Integer [1,100].");
            } else throw new Exception("Invalid tweaks.strength value. Expect String or Integer.");
    ...
    public static Point getCenter(RoutingContext context) throws Exception {
      double lat, lon;
      List<String> latParam = queryParam(LAT, context);
      List<String> lonParam = queryParam(LON, context);

      if (latParam.size() == 0 && lonParam.size() == 0) return null;
      if (latParam.size() == 1 && lonParam.size() == 0) throw new Exception("'" + LON + "' Param is missing!");
      if (lonParam.size() == 1 && latParam.size() == 0) throw new Exception("'" + LAT + "' Param is missing!");
      ...
      try {
        ...
        throw new Exception(
            "Invalid '" + LAT
                + "' query parameter, must be a WGS'84 longitude in decimal degree (so a value between -90 and +90).");
      }
      try {
