 ...
 */
public class JDBCSubscriptionConfigClient {
  //
  ...
  //    final SQLQueryExt query = new SQLQueryExt("SELECT config FROM " + SUBSCRIPTION_TABLE + " WHERE id = ?",
  // subscriptionId);
  ...
  //        }
  //      } else {
  //        logger.debug(marker, "subscriptionId[{}]: Failed to load configuration, reason: ", subscriptionId,
  // out.cause());
  //        p.fail(out.cause());
  //      }
  //    });
  ...
  //    final SQLQueryExt query = new SQLQueryExt("SELECT config FROM " + SUBSCRIPTION_TABLE + " WHERE source = ?",
  // source);
  //    client.queryWithParams(query.text(), new JsonArray(query.parameters()), out -> {
  //      if (out.succeeded()) {
  ...
  //    final SQLQueryExt query = new SQLQueryExt("INSERT INTO " + SUBSCRIPTION_TABLE + " (id, source, config) VALUES
  // (?, ?, cast(? as JSONB)) " +
  //        "ON CONFLICT (id) DO " +
  ...
  //    final SQLQueryExt query = new SQLQueryExt("DELETE FROM " + SUBSCRIPTION_TABLE + " WHERE id = ?",
  // subscriptionId);
  //    return get(marker, subscriptionId).compose(subscription -> updateWithParams(subscription,
  // query).map(subscription));
  //  }
  //
