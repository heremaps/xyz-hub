...
public abstract class CloudWatchMetricPublisher<V> extends MetricPublisher<V> {
  ...
  protected void publishValues(List<MetricDatum> data) {
    ...
    data.forEach(
        datum -> datum.withMetricName(getMetricName()).withUnit(unit).withDimensions(mainDimension));

    PutMetricDataRequest request =
        new PutMetricDataRequest().withNamespace(namespace).withMetricData(data);
  ...
  private static AmazonCloudWatch getClient() {
    if (_client == null) _client = AmazonCloudWatchAsyncClientBuilder.defaultClient();
    return _client;
  }
}
