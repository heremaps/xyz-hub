 ...
 */
public class JDBCConnectorConfigClient {
  //
  ...
  //
  //  @Override
  //  protected void getConnector(final Marker marker, final String connectorId, final Handler<AsyncResult<Connector>>
  // handler) {
  //    final SQLQueryExt query = new SQLQueryExt("SELECT config FROM " + CONNECTOR_TABLE + " WHERE id = ?",
  // connectorId);
  //    client.queryWithParams(query.text(), new JsonArray(query.parameters()), out -> {
  //      if (out.succeeded()) {
  //        final Optional<String> config = out.result().getRows().stream().map(r ->
  // r.getString("config")).findFirst();
  //        if (config.isPresent()) {
  ...
  //
  //  @Override
  //  protected void getConnectorsByOwner(Marker marker, String ownerId, Handler<AsyncResult<List<Connector>>>
  // handler) {
  //    final SQLQueryExt query = new SQLQueryExt("SELECT config FROM " + CONNECTOR_TABLE + " WHERE owner = ?",
  // ownerId);
  //    client.queryWithParams(query.text(), new JsonArray(query.parameters()), out -> {
  //      if (out.succeeded()) {
  ...
  //    final SQLQueryExt query = new SQLQueryExt("INSERT INTO " + CONNECTOR_TABLE + " (id, owner, config) VALUES (?,
  // ?, cast(? as JSONB)) " +
  //        "ON CONFLICT (id) DO " +
  ...
  //    final SQLQueryExt query = new SQLQueryExt("DELETE FROM " + CONNECTOR_TABLE + " WHERE id = ?", connectorId);
  ...
  //  }
  //
  //  private void updateWithParams(Connector modifiedObject, SQLQueryExt query, Handler<AsyncResult<Connector>>
  // handler) {
