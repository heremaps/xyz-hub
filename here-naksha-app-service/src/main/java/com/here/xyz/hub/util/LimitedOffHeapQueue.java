...
public class LimitedOffHeapQueue<E extends OffHeapBuffer> extends LimitedQueue<E> {
  ...
  private static final ScheduledExecutorService executors =
      new ScheduledThreadPoolExecutor(2, Core.newThreadFactory("oh-queues"));
  private static final OHCache<byte[], byte[]> ohStorage =
      OHCacheClient.createCache((int) (Service.configuration.GLOBAL_MAX_QUEUE_SIZE * 1.1), executors, true);
  ...
  public List<E> add(OffHeapBuffer element) {
    ...
    // Also discard the according off-heap elements explicitly. Otherwise the OHC would pick the elements to be
    // discarded.
  ...
  private static void discardOHElement(OffHeapBuffer element) {
    if (element.payload.get() == null && element.ohKey != null) ohStorage.remove(element.ohKey);
  }
  ...
  public static class OffHeapBuffer implements ByteSizeAware {
    ...
    private void unStash() throws PayloadVanishedException {
      ...
      if (tmpPayload == null) {
        if (ohKey == null) throw new IllegalStateException("Payload is not stashed. Can not un-stash it.");
        tmpPayload = ohStorage.get(ohKey);
        discardOHElement(this);
      }

      if (tmpPayload == null) throw new PayloadVanishedException(this);

      payload.set(tmpPayload);
    }

    public final byte[] getPayload() throws PayloadVanishedException {
      if (consumed.get()) throw new IllegalStateException("Payload was already consumed.");
    ...
    public final byte[] consumePayload() throws PayloadVanishedException {
      if (!consumed.compareAndSet(false, true)) throw new IllegalStateException("Payload was already consumed.");
