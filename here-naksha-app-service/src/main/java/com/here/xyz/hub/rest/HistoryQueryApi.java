...
public class HistoryQueryApi extends SpaceBasedApi {
  ...
  public void iterateHistory(final RoutingContext context) {
    try {
      ...
      if (context.parsedHeaders().accept().stream()
          .map(ParsedHeaderValue::rawValue)
          .anyMatch(APPLICATION_VND_HERE_COMPACT_CHANGESET::equals)) {
  ...
  public void getHistoryStatistics(final RoutingContext context) {
    new GetHistoryStatistics(
            new GetHistoryStatisticsEvent(),
            context,
            ApiResponseType.HISTORY_STATISTICS_RESPONSE,
            ApiParam.Query.getBoolean(context, SKIP_CACHE, false))
