...
public class SpaceApi extends SpaceBasedApi {
  ...
  public void getSpace(final RoutingContext context) {
    Map<String, Object> input = new JsonObject()
        .put("id", context.pathParam(ApiParam.Path.SPACE_ID))
        .getMap();
    ModifySpaceOp modifyOp =
        new ModifySpaceOp(Collections.singletonList(input), IfNotExists.ERROR, IfExists.RETAIN, true);
  ...
  public void postSpace(final RoutingContext context) {
    ...
    ModifySpaceOp modifyOp =
        new ModifySpaceOp(Collections.singletonList(input.getMap()), IfNotExists.CREATE, IfExists.ERROR, true);
  ...
  public void patchSpace(final RoutingContext context) {
    ...
    if (!input.getString("id").equals(pathId)) {
      context.fail(new HttpException(
          BAD_REQUEST, "The resource ID in the body does not match the resource ID in the path."));
      return;
    }

    ModifySpaceOp modifyOp =
        new ModifySpaceOp(Collections.singletonList(input.getMap()), IfNotExists.ERROR, IfExists.PATCH, true);
  ...
  public void deleteSpace(final RoutingContext context) {
    Map<String, Object> input =
        new JsonObject().put("id", context.pathParam(Path.SPACE_ID)).getMap();
    ModifySpaceOp modifyOp =
        new ModifySpaceOp(Collections.singletonList(input), IfNotExists.ERROR, IfExists.DELETE, true);

    // Delete the space
    ApiResponseType responseType = APPLICATION_JSON.equals(context.request().getHeader(ACCEPT))
        ? ApiResponseType.SPACE
