...
public class HealthApi extends Api {
  ...
  static {
    ...
    healthCheck.add(new MemoryHealthCheck()).add(new ClusterHealthCheck());
    ...
    if (Service.configuration.STORAGE_DB_URL != null) {
      healthCheck.add((ExecutableCheck) new JDBCHealthCheck(
              getStorageDbUri(),
              Service.configuration.STORAGE_DB_USER,
  ...
  public HealthApi(Vertx vertx, Router router) {
    ...
    router.route(HttpMethod.GET, "/")
        .handler(HealthApi::onHealthStatus); // TODO: Maybe better replace that one by a redirect to /hub/
    // Legacy.
  ...
  private static URI getPublicServiceEndpoint() {
    return URI.create(
        Service.configuration.XYZ_HUB_PUBLIC_ENDPOINT + Service.configuration.XYZ_HUB_PUBLIC_HEALTH_ENDPOINT);
  }

  private static URI getNodeHealthCheckEndpoint() {
    try {
      return new URI("http://" + Service.getHostname() + ":" + Service.configuration.HTTP_PORT
