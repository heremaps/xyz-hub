...
public class LogUtil {
  ...
  public static void addRequestInfo(RoutingContext context) {
    ...
    if (POST.equals(method) || PUT.equals(method) || PATCH.equals(method)) {
      accessLog.reqInfo.size =
          context.getBody() == null ? 0 : context.getBody().length();
    }
  ...
  public static AccessLog addResponseInfo(RoutingContext context) {
    ...
    if (tokenPayload != null) {
      ...
      String token =
          context.get(Query.ACCESS_TOKEN); // Could be a token ID or an actual JWT, both has to be classified
      if (token != null && token != "") accessLog.classified = new String[] {context.get(Query.ACCESS_TOKEN)};
    }
