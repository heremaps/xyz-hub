...
public class CWAttributedMetricCollectionPublisher<V> extends CloudWatchMetricPublisher<Map<Collection<Attribute>, V>> {
  ...
  protected void publishValues(Map<Collection<Attribute>, V> values) {
    List<MetricDatum> metricData = values.entrySet().stream()
        .map(e -> convertToMetricDatum(e.getKey(), e.getValue()))
  ...
  private MetricDatum convertToMetricDatum(Collection<Attribute> attributes, V value) {
    ...
    MetricDatum datum = new MetricDatum()
        .withDimensions(attributes.stream()
            .map(attr ->
                new Dimension().withName(attr.key.toString()).withValue(attr.value.toString()))
            .collect(Collectors.toList()));

    // Convert the metric value
    if (value instanceof AggregatedValues) {
      AggregatedValues aggregatedValues = (AggregatedValues) value;
      if (aggregatedValues.sampleCount == 0) return null;
      return datum.withStatisticValues(CWAggregatedValuesPublisher.toStatisticSet(aggregatedValues));
    } else if (value instanceof Collection) {
      if (((Collection<?>) value).size() == 0) return null;
      return datum.withValues((Collection<Double>) value);
    } else
      throw new IllegalArgumentException(
          "Can not publish metric value of type " + value.getClass().getSimpleName());
  }
}
