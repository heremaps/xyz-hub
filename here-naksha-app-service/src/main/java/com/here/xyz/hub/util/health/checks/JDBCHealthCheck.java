...
public class JDBCHealthCheck extends DBHealthCheck {
  ...
  public JDBCHealthCheck(String connectionStringEnvVar, String userEnvVar, String passwordEnvVar)
    ...
    try {
      ...
      if (result != 1) {
        setResponse(new Response()
            .withMessage(
                "Invalid result for sample query " + result + ". (Query: \"" + simpleQuery + "\")"));
        s.setResult(ERROR);
      } else {
        setResponse(null);
        s.setResult(OK);
      ...
      setResponse(new Response()
          .withMessage("Error when trying to do execute sample query. (Query: \"" + simpleQuery + "\")"));
      return s.withResult(ERROR);
    }
