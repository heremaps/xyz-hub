...
public class ApiParamTest {
  ...
  public void parsePropertiesQuery() {
    ...
    PropertyQueryOr pq = Query.parsePropertiesQuery(URIquery, "", false);
    assertEquals("1 OR block is expected", 1, pq.size());

    PropertyQueryAnd pql = pq.get(0);
  ...
  public void parsePropertiesQuerySpace() {
    ...
    PropertyQueryOr pq = Query.parsePropertiesQuery(URISpaceQuery, "contentUpatedAt", true);
    assertEquals("1 OR block is expected", 1, pq.size());
