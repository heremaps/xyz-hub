...
public final class PsqlPool implements AutoCloseable {
  ...
  @JsonIgnore public final @NotNull PsqlPoolConfig config;

  @JsonIgnore public final @NotNull HikariDataSource dataSource;
  ...
  private static synchronized @NotNull PsqlPool createAndCacheDataSource(
      final @NotNull PsqlPoolConfig config) {
    PsqlConnPoolRef poolRef;
    PsqlPool pool;
    poolRef = cache.get(config);
