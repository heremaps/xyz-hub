...
public class QuadbinSQL {
  ...
  public static SQLQuery generateQuadbinClusteringSQL(
    ...
    double bufferSizeInDeg =
        tile.getBBox(false).widthInDegree(true) / (Math.pow(2, resolution) * 1024.0);
    String realCountCondition = "",
        ...
        pureEstimation = "",
        resultQkGeo =
            (!noBuffer ? DhString.format("ST_Buffer(qkbbox, -%f)", bufferSizeInDeg) : "qkbbox"),
