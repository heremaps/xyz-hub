...
abstract class PsqlAbstractConfigBuilder<
    TARGET, SELF extends PsqlAbstractConfigBuilder<TARGET, SELF>> {
  ...
  public @NotNull SELF parseUrl(@NotNull String postgresUrl)
      throws URISyntaxException, ParameterError {
    ...
    if (!"postgresql".equalsIgnoreCase(uri.getScheme())) {
      throw new URISyntaxException(
          postgresUrl,
          "Expect scheme of specific part to be 'postgresql', but found: '"
              + uri.getScheme()
              + "'");
    }
