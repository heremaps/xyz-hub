...
import com.here.xyz.models.payload.events.tweaks.TweaksSampling.Algorithm;
...
public class SQLQueryBuilder {
  ...
  public static @NotNull SQLQuery buildGetStatisticsQuery(
      @NotNull PsqlHandler processor, boolean historyMode) {
    String function;
    ...
    return new SQLQuery(
        "SELECT * from " + schema + "." + function + "('" + schema + "','" + table + "')");
  }
  ...
  public static @NotNull SQLQuery buildGetFeaturesByBBoxQuery(
      final @NotNull GetFeaturesByBBoxEvent event) throws SQLException {
  ...
  private static int evalH3Resolution(
    ...
    if (clustering.getAbsoluteResolution() != null) {
      h3res =
          Math.min(
              clustering.getAbsoluteResolution(), defaultAbsoluteResForLevel + OVER_ZOOMING_RES);
    }
    ...
    h3res =
        Math.min(
            Math.min(h3res, defaultAbsoluteResForLevel + OVER_ZOOMING_RES),
            ClusteringHexBin.MAX_RESOLUTION);
    return h3res;
  }
  ...
  public static SQLQuery buildHexbinClusteringQuery(
      ...
      @NotNull BBox bbox,
      @NotNull ClusteringHexBin clustering) {
    ...
    final boolean statisticalPropertyProvided =
        clustering.property != null && clustering.property.length() > 0;
    ...
    final String fid =
        (!event.getClip() ? "h3" : DhString.format("h3 || %f || %f", bbox.minLon(), bbox.minLat()));
    final String filterEmptyGeo =
  ...
  public static SQLQuery buildSamplingTweaksQuery(
    ...
    final @Nullable TweaksEnsure ensure =
        tweaks instanceof TweaksEnsure ? (TweaksEnsure) tweaks : null;
    final Algorithm algorithm = tweaks.algorithm();
    final float tblSampleRatio =
        tweaks.sampling.strength > 0 && algorithm.distribution2
            ? TweaksSQL.tableSampleRatio(tweaks.sampling.strength)
            : -1f;
    final String sCondition =
        tblSampleRatio >= 0.0f || ensure != null || tweaks.sampling.strength == 0
            ? "1 = 1"
    ...
    return generateCombinedQueryTweaks(
        event, tweakQuery, tweaksGeoSql, false, tblSampleRatio, bSortByHashedValue);
  }
  ...
  public static SQLQuery buildSimplificationTweaksQuery(
      @NotNull GetFeaturesByBBoxEvent event,
      @NotNull BBox bbox,
      @NotNull TweaksSimplification simplification)
      throws SQLException {
    ...
    if (event.getClip()
        && algorithm != TweaksSimplification.Algorithm.GRID_BY_TILE_LEVEL
        && bMvtRequested) {
    ...
    if (algorithm.pgisAlgorithm != null) {
      final double tolerance =
          simplification.sampling.strength > 0
    ...
    if (algorithm == TweaksSimplification.Algorithm.MERGE) {
      query =
          new SQLQuery(
              format(TweaksSQL.mergeBeginSql, tweaksGeoSql, minGeoHashLenToMerge, bboxqry));
    } else {
  ...
  protected static SQLQuery generateIDXStatusQuery(
      final @NotNull String schema, final @NotNull String table) {
  ...
  public static SQLQuery buildRemoveSubscriptionQuery(
      @NotNull String spaceId, @NotNull String schema) {
    String theSql =
