...
public class GetFeaturesByGeometry extends Spatial<GetFeaturesByGeometryEvent> {
  ...
  public GetFeaturesByGeometry(
      @NotNull GetFeaturesByGeometryEvent event, @NotNull PsqlHandler psqlConnector)
      throws SQLException {
    super(event, psqlConnector);
  }

  @Override
  protected @NotNull SQLQuery buildQuery(@Nonnull GetFeaturesByGeometryEvent event)
      throws SQLException {
    final int radius = event.getRadius();

    SQLQuery geoFilter =
        event.getH3Index() != null
            ? new SQLQuery("(h3ToGeoBoundaryDeg(('x' || #{h3Index})::bit(60)::bigint))")
                .withNamedParameter("h3Index", event.getH3Index())
  ...
  @Override
  protected SQLQuery buildClippedGeoFragment(
      final GetFeaturesByGeometryEvent event, SQLQuery geoFilter) {
    if (!event.getClip()) return super.buildGeoFragment(event);

    SQLQuery clippedGeo =
        new SQLQuery("ST_Intersection(ST_MakeValid(geo), ${{geoFilter}})")
            .withQueryFragment("geoFilter", geoFilter);
