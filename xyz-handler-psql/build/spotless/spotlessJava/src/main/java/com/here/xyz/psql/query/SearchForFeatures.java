...
public class SearchForFeatures<E extends SearchForFeaturesEvent> extends GetFeatures<E> {
  ...
  public SearchForFeatures(@NotNull E event, @NotNull PsqlHandler psqlConnector)
      throws SQLException {
              ...
              propertyQuery -> {
                ...
                for (int i = 0; i < valuesCount; i++) {
                  ...
                  if (v == null) {
                    ...
                    SQLQuery q1 =
                        new SQLQuery(op.equals(QueryOperation.NOT_EQUALS) ? "((" : "not ((");
                    q1.append(q);
  ...
  private static SQLQuery generateTagsQuery(TagsQuery tagsQuery) {
    ...
    for (int i = 0; i < tagsQuery.size(); i++) {
      query.append(
          (i == 0 ? "" : " OR ")
              + "jsondata->'properties'->'@ns:com:here:xyz'->'tags' ??& #{tags"
              + i
              + "}");
  ...
  protected static SQLQuery generateSearchQuery(
      final QueryEvent event) { // TODO: Make private again
