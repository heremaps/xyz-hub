...
public abstract class GetFeatures<E extends Event> extends ExtendedSpace<E, FeatureCollection> {
  ...
  public GetFeatures(@NotNull E event, final @NotNull PsqlHandler psqlConnector)
      throws SQLException {
  ...
  protected @NotNull SQLQuery buildQuery(@Nonnull E event) throws SQLException {
    ...
    query.setQueryFragment(
        "iColumn", ""); // NOTE: This can be overridden by implementing subclasses
    ...
    if (isExtended) {
      query.setQueryFragment(
          "iColumnExtension", ""); // NOTE: This can be overridden by implementing subclasses
      query.setQueryFragment("iOffsetExtension", "");

      SQLQuery baseQuery =
          !is2LevelExtendedSpace(event)
              ? build1LevelBaseQuery(getExtendedTable(event)) // 1-level extension
              : build2LevelBaseQuery(
                  getIntermediateTable(event), getExtendedTable(event)); // 2-level extension

      query.setQueryFragment("baseQuery", baseQuery);

      query.setQueryFragment(
          "iColumnBase", ""); // NOTE: This can be overridden by implementing subclasses
      query.setQueryFragment(
          "iOffsetBase", ""); // NOTE: This can be overridden by implementing subclasses
      query.setQueryFragment(
      ...
      query.setQueryFragment(
          "orderBy", ""); // NOTE: This can be overridden by implementing subclasses
      query.setQueryFragment(
          "offset", ""); // NOTE: This can be overridden by implementing subclasses
    }
  ...
  private SQLQuery build1LevelBaseQuery(String extendedTable) {
    SQLQuery query =
        new SQLQuery(
            "SELECT jsondata, geo${{iColumnBase}}"
  ...
  protected static SQLQuery buildGeoFragment(
      Event event, boolean convertToGeoJson, SQLQuery geoOverride) {
    boolean isForce2D = event instanceof FeatureEvent && ((FeatureEvent) event).isForce2D();
    String geo =
        geoOverride != null
            ? "${{geoOverride}}"
            : ((isForce2D ? "ST_Force2D" : "ST_Force3D") + "(geo)");
    ...
    if (convertToGeoJson)
      geo =
          "replace(ST_AsGeojson("
              + geo
              + ", "
              + SQLQueryBuilder.GEOMETRY_DECIMAL_DIGITS
              + "), 'nan', '0')";
