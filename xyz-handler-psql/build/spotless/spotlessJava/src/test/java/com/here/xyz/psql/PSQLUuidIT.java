...
public class PSQLUuidIT extends PSQLAbstractIT {
  ...
  protected void testModifyFeatureFailures(boolean withUUID) throws Exception {
    ...
    String insertJsonFile =
        withUUID
            ? "/events/InsertFeaturesEventTransactional.json"
            : "/events/InsertFeaturesEvent.json";
    ...
    if (withUUID) {
      assertEquals(
          DatabaseWriter.DELETE_ERROR_UUID, responseCollection.getFailed().get(0).getMessage());
    } else {
      assertEquals(
    ...
    assertEquals(
        DatabaseWriter.INSERT_ERROR_GENERAL, responseCollection.getFailed().get(0).getMessage());
    assertEquals(0, responseCollection.getFeatures().size());
    assertNull(responseCollection.getUpdated());
    assertNull(responseCollection.getInserted());
    assertNull(responseCollection.getDeleted());
    ...
    if (withUUID) {
      assertEquals(
          DatabaseWriter.UPDATE_ERROR_UUID, responseCollection.getFailed().get(0).getMessage());
    } else {
      assertEquals(
