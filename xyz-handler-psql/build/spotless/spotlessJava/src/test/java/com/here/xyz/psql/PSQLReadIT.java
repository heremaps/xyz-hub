...
public class PSQLReadIT extends PSQLAbstractIT {
  ...
  public void testBBoxQuery() throws Exception {
    ...
    final String insertRequest =
        IOUtils.toString(this.getClass().getResourceAsStream(insertJsonFile));
    ...
    // getFeaturesByBBoxEvent.setConnectorParams(defaultTestConnectorParams);
    // getFeaturesByBBoxEvent.setSpaceId("foo");
    getFeaturesByBBoxEvent.setTags(
        TagsQuery.fromQueryParameter(
    ...
    getFeaturesByBBoxEvent.setSelection(
        new ArrayList<String>() {
          {
            add("id");
            add("type");
    ...
    assertEquals(1, features.size());
    assertEquals(
        1, new ObjectMapper().convertValue(features.get(0).getProperties(), Map.class).size());
    ...
    assertEquals(1, features.size());
    assertEquals(
        2, new ObjectMapper().convertValue(features.get(0).getProperties(), Map.class).size());
  ...
  public void testGetStatisticsEvent() throws Exception {
                    ...
                    () -> {
                      ...
                      f.setGeometry(
                          new Point()
                              .withCoordinates(
                                  new PointCoordinates(
                                      360d * RANDOM.nextDouble() - 180d,
                                      180d * RANDOM.nextDouble() - 90d)));
                      f.getProperties().setXyzNamespace(xyzNamespace);
                      pKeys.forEach(
                          p -> f.getProperties().put(p, RandomStringUtils.randomAlphanumeric(8)));
                      return f;
                    })
                .limit(11000)
                .collect(Collectors.toList()));
    ...
    mfevent.setTransaction(true);
    mfevent.setEnableUUID(true);
    mfevent.setInsertFeatures(
        collection.getFeatures()); // TODO use get11kFeatureCollection() and extract pKeys
  ...
  public void testSearchAndCountByPropertiesAndTags() throws Exception {
    ...
    String basic =
        IOUtils.toString(
            this.getClass()
                .getResourceAsStream("/events/BasicSearchByPropertiesAndTagsEvent.json"));
  ...
  protected void invokeAndAssert(Map<String, Object> json, int size, String... names)
      throws Exception {
