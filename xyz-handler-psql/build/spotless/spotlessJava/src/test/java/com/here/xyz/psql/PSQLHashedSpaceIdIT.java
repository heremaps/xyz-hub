...
public class PSQLHashedSpaceIdIT extends PSQLAbstractIT {
  ...
  public void testTableCreation() throws Exception {
    ...
    final XyzNamespace xyzNamespace =
        new XyzNamespace().withSpace("foo").withCreatedAt(1517504700726L);
  ...
  public void testAutoIndexing() throws Exception {
    ...
    try (final Connection connection = dataSource().getConnection()) {
      ...
      try (ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM xyz_config.xyz_idxs_status where spaceid = '"
                  + hashedSpaceId
                  + "';")) {
        assertTrue(rs.next());
      ...
      try (ResultSet rs =
          stmt.executeQuery(
              "SELECT * FROM pg_indexes WHERE tablename = '" + hashedSpaceId + "';")) {
        ...
        indexes.addAll(
            features.get(0).getProperties().additionalProperties().keySet().stream()
                .filter(k -> !"test".equals(k))
                .filter(k -> !"@ns:com:here:xyz".equals(k))
                .map(
                    k ->
                        "idx_" + hashedSpaceId + "_" + DigestUtils.md5Hex(k).substring(0, 7) + "_a")
        ...
        assertFalse(
            extractedIndexes.contains(
                "idx_" + hashedSpaceId + "_" + DigestUtils.md5Hex("test").substring(0, 7) + "_a"));
      }
