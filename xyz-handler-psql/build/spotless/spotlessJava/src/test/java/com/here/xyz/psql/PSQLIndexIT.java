...
public class PSQLIndexIT extends PSQLAbstractIT {
  ...
  static Map<String, Object> connectorParams =
  ...
  public void testOnDemandIdxLimit() throws Exception {
    Map<String, Boolean> searchableProperties =
    ...
    assertEquals(
        "On-Demand-Indexing - Maximum permissible: 4 searchable properties per space!",
    ...
    SuccessResponse response =
        JsonSerializable.deserialize(invokeLambda(modifySpaceEvent.serialize()));
    ...
    try (final Connection connection = dataSource().getConnection()) {
      ...
      String sqlSpaceSchema =
          "(select schema_name::text from information_schema.schemata where schema_name in ('xyz','public') order by 1 desc limit 1)";

      Statement stmt = connection.createStatement();
      stmt.execute(
          DhString.format("select xyz_maintain_idxs_for_space( %s, 'foo');", sqlSpaceSchema));

      /** Check which Indices are available */
      ResultSet resultSet =
          stmt.executeQuery(
              DhString.format(
                  "select idx_name, idx_property, src from xyz_index_list_all_available(%s, 'foo');",
                  sqlSpaceSchema));
  ...
  public void testOnDemandIdxCreation() throws Exception {
    ...
    try (final Connection connection = dataSource().getConnection()) {
      ...
      ResultSet resultSet =
          stmt.executeQuery(
              DhString.format(
                  "select idx_property, src from xyz_index_list_all_available(%s, 'foo');",
                  sqlSpaceSchema));
      while (resultSet.next()) {
        String idxProperty = resultSet.getString("idx_property");
        if (systemIndices.contains(idxProperty)) {
    ...
    StatisticsResponse resp =
        JsonSerializable.deserialize(invokeLambda(statisticsEvent.serialize()));
    assertEquals(
        StatisticsResponse.PropertiesStatistics.Searchable.ALL,
        resp.getProperties().getSearchable());
        ...
        switch (idx_property) {
          case "foo":
            assertEquals(
                "CREATE INDEX "
                    + idx_name
                    + " ON public.foo USING btree ((((jsondata -> 'properties'::text) -> 'foo'::text)))",
                indexdef);
            break;
          case "foo2":
            assertEquals(
                "CREATE INDEX "
                    + idx_name
                    + " ON public.foo USING gin ((((jsondata -> 'properties'::text) -> 'foo2'::text)))",
                indexdef);
            break;
          case "foo.nested":
            assertEquals(
                "CREATE INDEX "
                    + idx_name
                    + " ON public.foo USING btree (((((jsondata -> 'properties'::text) -> 'foo'::text) -> 'nested'::text)))",
                indexdef);
            break;
          case "f.fooroot":
            assertEquals(
                "CREATE INDEX "
                    + idx_name
                    + " ON public.foo USING btree (((jsondata -> 'fooroot'::text)))",
                indexdef);
            break;
          case "f.geometry.type":
            assertEquals(
                "CREATE INDEX " + idx_name + " ON public.foo USING btree (geometrytype(geo))",
                indexdef);
            break;
          case "id":
            assertEquals(
                "CREATE UNIQUE INDEX idx_foo_id ON public.foo USING btree (((jsondata ->> 'id'::text)))",
                indexdef);
            break;
          case "geo":
  ...
  public void testAutoIndexing() throws Exception {
    ...
    List<StatisticsResponse.PropertyStatistics> propStatistics =
