...
public class PSQLWriteIT extends PSQLAbstractIT {
  ...
  public void testTableCreated() throws Exception {
    ...
    assertEquals(
        "Check response status", JsonPath.read(response, "$.type").toString(), "FeatureCollection");
  }
  ...
  public void testCrudFeatureWithoutHash() throws Exception {
    ...
    String searchPropertiesResponse =
        invokeLambdaFromFile("/events/SearchForFeaturesByPropertiesEvent.json");
    ...
    String featuresList =
        JsonSerializable.serialize(
            featureCollection.getFeatures(), new TypeReference<List<Feature>>() {});
  ...
  protected void assertUpdate(String updateRequest, String response, boolean checkGuid)
      throws Exception {
    ...
    for (int i = 0; i < gsModifyFeaturesEvent.getUpdateFeatures().size(); i++) {
      ...
      assertEquals(
          ...
          actualFeature.getProperties().get("name"));
      assertNotNull(actualFeature.getId());

      assertTrue(
          jsonCompare(
              expectedFeature.getProperties().getXyzNamespace().getTags(),
      ...
      if (checkGuid) {
        assertNotNull(
            actualFeature.getProperties().getXyzNamespace().getUuid()); // After version 0.2.0
  ...
  protected void assertCount(String insertRequest, String countResponse) {
    if (!JsonPath.<Boolean>read(countResponse, "$.count.estimated")) {
      assertEquals(
          "Check inserted feature count vs fetched count",
          JsonPath.read(insertRequest, "$.insertFeatures.length()").toString(),
  ...
  protected void testModifyFeatures(boolean includeOldStates) throws Exception {
    ...
    final DocumentContext updateFeaturesEventDoc =
        getEventFromResource("/events/InsertFeaturesEvent.json");
    updateFeaturesEventDoc.put(
        "$", "params", Collections.singletonMap("includeOldStates", includeOldStates));
    ...
    if (includeOldStates) {
      ...
      assertTrue(
          oldFeatures.size() > 0, "'oldFeatures' element in ModifyFeaturesResponse is empty");
    ...
    // =========== DELETE ==========
    final DocumentContext modifyFeaturesEventDoc =
        getEventFromResource("/events/InsertFeaturesEvent.json");
    modifyFeaturesEventDoc.put(
        "$", "params", Collections.singletonMap("includeOldStates", includeOldStates));
