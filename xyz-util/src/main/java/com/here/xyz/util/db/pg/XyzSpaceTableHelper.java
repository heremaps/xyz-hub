...
//              + "CONSTRAINT ${uniqueConstraintName} UNIQUE (id, next_version), "
  ...
  /**
   * The prefix that is used for all physical table names which were generated independently of the space ID.
   * It guarantees that the generated identifier never starts with a digit and makes such tables recognizable.
   */
  public static final String GENERATED_TABLE_NAME_PREFIX = "s_";

  /**
   * Generates a new, unique physical table name which is completely independent of the space ID.
   * <p>
   * The resulting name is {@value #GENERATED_TABLE_NAME_PREFIX} + a 32 character hex hash of a random UUID,
   * so 34 characters in total.
   *
   * @return A new unique physical table name
   */
  public static String generateUniqueTableName() {
    return GENERATED_TABLE_NAME_PREFIX + Hasher.getHash(UUID.randomUUID().toString());
  }

  /**
   * @param tableName The physical table name to be checked
   * @return Whether the specified table name was generated by {@link #generateUniqueTableName()}
   */
  public static boolean isGeneratedTableName(String tableName) {
    return tableName != null && tableName.startsWith(GENERATED_TABLE_NAME_PREFIX)
        && tableName.length() == GENERATED_TABLE_NAME_PREFIX.length() + 32;
  }
