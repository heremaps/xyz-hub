 ...
 */
CREATE OR REPLACE FUNCTION import_from_s3_get_work_item(temporary_tbl REGCLASS)
 ...
 */
CREATE OR REPLACE FUNCTION import_from_s3_report_success(temporary_tbl REGCLASS, success_callback TEXT)
    RETURNS void
    LANGUAGE 'plpgsql'
...
/**
 * Start import from S3
 */
CREATE OR REPLACE PROCEDURE import_from_s3_start(schem TEXT, temporary_tbl REGCLASS, target_tbl REGCLASS, format TEXT,
...
DECLARE
    ...
    sql_text TEXT;
BEGIN
    SELECT * from import_from_s3_get_work_item(temporary_tbl) into work_item;
    COMMIT;
    ...
    ELSE
        IF work_item.state = 'RETRY' OR work_item.state = 'LAST_ONES_RUNNING' THEN
            ...
            PERFORM pg_sleep(10);
            PERFORM asyncify(format('CALL import_from_s3_start(%1$L,  %2$L,  %3$L, %4$L, %5$L, %6$L);',
        ...
        ELSEIF work_item.state = 'SUCCESS_MARKER_RUNNING' THEN
            EXECUTE format('SELECT import_from_s3_report_success(%1$L,%2$L);', temporary_tbl, success_callback);
            RETURN;
        END IF;
    END IF;
    ...
    BEGIN
			BEGIN
	            ...
	            IF work_item_s3_bucket != 'SUCCESS_MARKER' THEN
					PERFORM import_from_s3_perform(schem, temporary_tbl, target_tbl, work_item_s3_bucket ,work_item_s3_path, work_item_s3_region,
