...
class FeatureWriter {
    ...
    if (this.onVersionConflict != null) {
      ...
      if (updatedRows.length == 1) {
        ...
        if (this.tables.length > 1) {
           ...
           */
          existingFeature = this.loadFeature(this.inputFeature.id, "HEAD", "SUPER"); //FIXME: Should not simply be "HEAD" here for branching: instead it should be checking if the feature exists anywhere in 0..<tableBaseVersions.at(-1)>
          featureExists = !!existingFeature;
        }
        ...
        if (this.operation != "D") {
          if (featureExists && !FeatureWriter._isComposite())
            this.operation = "U";
          return this._insertHistoryRow(existingFeature);
        }
      }
    }
  }
  ...
  static getNextVersion() {
    ...
    return plv8.execute("SELECT nextval($1)", [fullQualifiedSequenceName])[0].nextval + this._tableBaseVersions().at(-1);
  }
   ...
   * @returns {FeatureModificationExecutionResult}
   */
  _insertHistoryRow(existingFeature = null, resultHandler = IDENTITY_HANDLER) {
    return FeatureWriter.dbWriter.insertHistoryRow(this.inputFeature, existingFeature, this.version - this.tableBaseVersions.at(-1), this.operation, this.author, resultHandler);
  }
