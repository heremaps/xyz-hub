...
public abstract class OldEventHandler implements IEventHandler {
  ...
  @SuppressWarnings("rawtypes")
  private static final ConcurrentHashMap<String, Constructor> allConstructors =
  ...
  public static <H extends OldEventHandler> @NotNull H newInstance(@NotNull Connector connector)
    ...
    if (id == null) {
      throw new XyzErrorException(
          XyzError.ILLEGAL_ARGUMENT, "The given connector does not have an 'id'");
    }
    ...
    if (handlerClass == null || constructor == null) {
      throw new XyzErrorException(
          XyzError.ILLEGAL_ARGUMENT, "No such event handler (" + id + ") exists");
    }
    try {
