...
public class XyzNamespace extends JsonObject {
   ...
   */
  public static @NotNull String normalizeTag(
      final @NotNull String tag, @Nullable StringBuilder sb) {
    if (tag.length() == 0) {
      return tag;
    }
    ...
    final char[] MAP =
        first == '~'
                ...
                || normalized.startsWith("sourceID_")
            ? AS_IS
            : TO_LOWER;
    if (sb == null) {
   ...
   */
  public static @Nullable List<@NotNull String> normalizeTags(
      final @Nullable List<@NotNull String> tags) {
    final int SIZE;
