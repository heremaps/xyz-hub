...
public abstract class Geometry implements Typed {
  ...
  @JsonIgnore private com.vividsolutions.jts.geom.Geometry geomCache;
  ...
  public static <T extends Geometry> T convertJTSGeometry(
   ...
   */
  public static void validateMultiPointCoordinates(final Object raw)
    ...
    for (int i = 0; i < points.size(); i++) {
      ...
      if (!(point instanceof List)) {
        throw new InvalidGeometryException(
            "Expected Point at index #" + i + " in the MultiPoint coordinates");
      }
   ...
   */
  public static void validateLineStringCoordinates(final Object raw)
      throws InvalidGeometryException {
    if (!(raw instanceof List)) {
      throw new InvalidGeometryException("Coordinates are expected to be an array");
    }
    ...
    if (points.size() < 2) {
      throw new InvalidGeometryException(
          "Coordinates of a LineString must have at least two positions");
    }
    for (int i = 0; i < points.size(); i++) {
      final Object point = points.get(i);
      if (!(point instanceof List)) {
        throw new InvalidGeometryException(
            "Expected Point at index #" + i + " in the LineString coordinates");
      }
      validatePointCoordinates(point);
    }
  }
   ...
   *     coordinates.
   */
  public static void validateMultiLineStringCoordinates(final Object raw)
      throws InvalidGeometryException {
    if (!(raw instanceof List)) {
      throw new InvalidGeometryException("Coordinates are expected to be an array");
    }
  ...
  public static void validateLinearRingCoordinates(final Object raw, boolean isExterior)
    ...
    if (points.size() < 4) {
      throw new InvalidGeometryException(
          "Coordinates of a LinearRing must have at least four positions");
    }
    //
    ...
    for (int i = 0; i < points.size(); i++) {
      ...
      if (!(point instanceof List)) {
        throw new InvalidGeometryException(
            "Expected Point at index #" + i + " in the LinearRing coordinates");
      }
      validatePointCoordinates(point);
    }
   ...
   */
  public static void validateMultiPolygonCoordinates(final Object raw)
      throws InvalidGeometryException {
    if (!(raw instanceof List)) {
      throw new InvalidGeometryException("Coordinates are expected to be an array");
    }
