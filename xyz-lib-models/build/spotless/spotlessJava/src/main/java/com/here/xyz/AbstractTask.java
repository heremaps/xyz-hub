...
public abstract class AbstractTask<EVENT extends Event> implements UncaughtExceptionHandler {
  ...
  public static final AtomicReference<@Nullable Supplier<@NotNull AbstractTask<?>>> FACTORY =
      new AtomicReference<>();
  ...
  public void attachToCurrentThread() {
    ...
    final UncaughtExceptionHandler threadUncaughtExceptionHandler =
  ...
  protected final void lock() {
    ...
    if (currentState != State.NEW) {
      ...
      throw new IllegalStateException(
          "Found illegal state " + currentState.name() + ", expected NEW");
    }
  }
      ...
      do {
        ...
        if (!internal && threadCount >= LIMIT) {
          throw new XyzErrorException(
              XyzError.TOO_MANY_REQUESTS,
              "Maximum number of concurrent requests (" + LIMIT + ") reached");
        }
  ...
  private final @NotNull List<@NotNull Consumer<@NotNull XyzResponse>> listeners =
      new ArrayList<>();
  ...
  protected @NotNull XyzResponse errorResponse(@NotNull Throwable t) {
    ...
    if (t instanceof XyzErrorException e) {
      ...
      response =
          errorResponse(
              XyzError.EXCEPTION, "Uncaught exception in task " + getClass().getSimpleName());
    }
    return response;
  }
   ...
   * @return the error response.
   */
  protected final @NotNull XyzResponse errorResponse(
      @NotNull XyzError error, @NotNull String message) {
