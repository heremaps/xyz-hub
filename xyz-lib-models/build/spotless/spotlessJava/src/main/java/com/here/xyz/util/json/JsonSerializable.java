...
public interface JsonSerializable {
  ...
  static @NotNull String serialize(
      @Nullable Object object, @NotNull TypeReference<?> typeReference) {
    try (final Json json = Json.open()) {
  ...
  static <OBJECT extends JsonSerializable> @NotNull OBJECT deepClone(@NotNull OBJECT object) {
    ...
    try (final Json json = Json.open()) {
      ...
      final Object clone =
          json.reader(Deserialize.Any.class).forType(object.getClass()).readValue(bytes);
      return (OBJECT) clone;
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  ...
  static <T> @NotNull T fromAnyMap(
      @NotNull Map<@NotNull String, @Nullable Object> map, @NotNull Class<T> klass) {
    try (final Json json = Json.open()) {
      return json.mapper.convertValue(map, klass);
    }
  }
