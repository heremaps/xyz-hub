...
public class WebMercatorTile {
  ...
  public static WebMercatorTile getTileFromLatLonLev(double lat, double lon, int lev) {
    double sinLatitude = Math.sin(lat * Math.PI / 180),
        ...
        col =
            Math.floor(
                (.5 - Math.log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI))
                    * Math.pow(2, lev));
   ...
   *     too short/long.
   */
  public static long quadKeyToTile(String quadKey)
  ...
  public PreparedGeometry getAsPolygon() {
    ...
    final Envelope envelope =
        new Envelope(bbox.minLon(), bbox.maxLon(), bbox.minLat(), bbox.maxLat());
