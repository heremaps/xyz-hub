...
public class Patcher {
  ...
  public static @Nullable Difference getDifference(
    ...
    if ((sourceState instanceof Number sourceNumber)
        && (targetState instanceof Number targetNumber)) {
  ...
  private static MapDiff getMapDifference(
    ...
    for (final @NotNull Object key : keysB) {
      if (sourceState.containsKey(key)
          || ignoreKey != null && ignoreKey.ignore(key, sourceState, targetState)) {
        continue;
      }
   ...
   */
  private static ListDiff mergeListDifferences(
      ListDiff diffA, ListDiff diffB, ConflictResolution cr) throws MergeConflictException {
    ...
    try {
      ...
      throw new MergeConflictException(
          "Unexpected exception while merging:\n" + ExceptionUtils.getStackTrace(e));
    }
    return mergeDiff;
  }
    ...
    for (final Object key : keys) {
      ...
      if (diff instanceof InsertOp) {
        ...
        throw new IllegalStateException(
            "The given map contains at key '" + key + "' an invalid element");
      }
    }
  }
    ...
    try {
      for (int i = listDiff.size() - 1; i >= 0; i--) {
        ...
        if (diff instanceof UpdateOp) {
          ...
          throw new IllegalStateException(
              "The given list contains at index #" + i + " an invalid element");
        }
      }
