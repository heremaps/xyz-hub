...
public class EventPipeline implements IEventContext {
  ...
  public @NotNull XyzResponse sendUpstream() {
    ...
    if (handler == null) {
      currentLogger()
          .error("Pipeline handler[{}] is null, skip it", next - 1, new NullPointerException());
  ...
  private @NotNull XyzResponse pipelineEnd(@NotNull IEventContext eventContext) {
    currentLogger()
        .info(
            "End of pipeline reached and no handle created a response", new NullPointerException());
    return notImplemented();
  }
