...
import com.here.xyz.models.RemoteInvocation;
...
/**
 * A remote event handler is an artificially deployed component of Naksha, which runs in an own
 * process, accessed using a URL. All events are “POST” (see {@link #method}) to the URL and wrapped
 * into an {@link RemoteInvocation} object. The handler need to return either an {@link Event} or an
 * {@link XyzResponse}, which both extend {@link Payload}. If an {@link Event} is returned, this
 * event is send upstream in the event-pipeline. If an {@link XyzResponse} is returned, this
 * response will be returned. Naksha provides everything that is needed to create artificial
 * extensions.
 */
...
public class RemoteEventHandler extends EventHandler {
  ...
  public static final String HTTP_PROXY_CLASS_NAME = "com.here.naksha.HttpEventHandlerProxy";
  ...
  public RemoteEventHandler(
      @JsonProperty(ID) @NotNull String id, @JsonProperty(URL) @NotNull String url)
  ...
  /**
   * The name of the cookies to forward to the event handler. To set this property special rights
   * are requires.
   */
  @AvailableSince(INaksha.v2_0)
  ...
  /**
   * The name of the HTTP headers to forward to the event handler. To set this property special
   * rights are requires.
   */
  @AvailableSince(INaksha.v2_0)
  ...
  public @NotNull IEventHandler newInstance() throws Exception {
    try {
      // TODO: If the extension is remote, create an HTTP proxy instead of creating the extension
      // directly!
      //       The HTTP proxy will need the connector and this extension object, and will then, when
      ...
      throw new UnsupportedOperationException(
          "HTTP Proxy implementation for remote extensions missing");
