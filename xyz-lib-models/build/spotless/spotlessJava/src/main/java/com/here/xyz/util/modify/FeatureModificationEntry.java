...
public class FeatureModificationEntry<FEATURE extends Feature> {
   ...
   * {@link XyzNamespace}.
   */
  protected static final HashMap<@NotNull String, @NotNull Boolean> xyzNamespaceIgnore =
      new HashMap<>() {
        {
  ...
  protected boolean ignore(
      @NotNull Object key, @NotNull Object source, @NotNull Object targetOrPatch) {
  ...
  final @Nullable Action apply() throws ModificationException, MergeConflictException {
    ...
    assert false;
    final var e =
        new ModificationException(
            "Reached unexpected step while processing feature {" + input.getId() + "}");
