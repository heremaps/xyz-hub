...
public abstract class MapEntrySet<K, V, M extends Map<@NotNull K, V>>
  ...
  @JsonIgnore protected final @NotNull M map;

  @JsonIgnore protected final @NotNull Class<K> keyClass;

  @JsonIgnore protected final @NotNull Class<V> valueClass;
  ...
  public <T> @Nullable T @NotNull [] toArray(@Nullable T @NotNull [] a) {
    return (T[])
        JsonUtils.mapToArray(map, a, (e, m) -> new MapEntry<>(map, e.getKey(), e.getValue()));
  }
