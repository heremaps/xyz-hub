...
public class IoEventPipeline extends EventPipeline {
      ...
      if (!(typed instanceof Event)) {
        ...
        response =
            new ErrorResponse()
                ...
                .withErrorMessage(
                    "Invalid event, expected " + expected + ", but found " + deserialized);
        if (output != null) {
  ...
  private void writeDataOut(
    ...
    try {
      ...
      currentLogger()
          .info("Write data out for response with type: {}", dataOut.getClass().getSimpleName());
        ...
        if (dataOut instanceof BinaryResponse) {
          ...
          if (XyzResponse.etagMatches(ifNoneMatch, etag)) {
            ...
            try (final ByteArrayOutputStream os =
              ...
              currentLogger()
                  .error(
                      "Unexpected exception while trying to generate response bytes with e-tag", e);
            }
          }
        }
      }
