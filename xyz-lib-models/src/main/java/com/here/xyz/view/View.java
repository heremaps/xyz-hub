...
/**
 * Properties that are not bound to views, will always be part of the serialization and deserialization. When properties are linked to views
 * using the {@link JsonView} annotation, then they have three distinct capabilities. How they are exported to clients, how they are
 * imported from clients and if they are persisted into a storage.
 * <p>
 * <b>Warning</b>: Control can be very fine grained, so to make a virtual read-only property, that can only be read by clients, but does
 * not exist in the database nor is available otherwise internally, do this: <pre>{@code
 ...
 * }
 * }</pre>
 * This will result in the property being part of the public REST API, but the client can't set it, and it is not serialized internally. It
 * is recommended to not add the {@link JsonView} annotation, except some special handling needed. In that case, whenever possible, use the
 * default {@link View.Public}, {@link View.Protected} or {@link View.Private} annotations.
 */
