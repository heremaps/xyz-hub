...
/**
 * The "hexbin" algorithm divides the world in hexagonal "bins" on a specified resolution. Each hexagon has an address described by the H3
 * addressing scheme. For more information on this topic see: <a href="https://eng.uber.com/h3/">https://eng.uber.com/h3/</a>.
 */
...
public class ClusteringHexBin extends Clustering {
  ...
  /**
   * The optional H3 hexagon resolution [0,13]. This parameter is as well available with the downward compatible naming “resolution”.
   */
  private @Nullable Integer absoluteResolution;

  public @Nullable Integer getAbsoluteResolution() {
    if (absoluteResolution != null && (absoluteResolution < MIN_RESOLUTION || absoluteResolution > MAX_RESOLUTION)) {
      return null;
    }
  ...
  public void setAbsoluteResolution(@Nullable Integer absoluteResolution) throws ParameterError {
    if (absoluteResolution != null && (absoluteResolution < MIN_RESOLUTION || absoluteResolution > MAX_RESOLUTION)) {
      throw new ParameterError(format("absoluteResolution must be %d to %d and was: %d", MIN_RESOLUTION, MAX_RESOLUTION, absoluteResolution));
    }
