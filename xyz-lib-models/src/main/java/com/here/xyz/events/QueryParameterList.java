...
public class QueryParameterList implements Iterable<QueryParameter> {
   ...
   * @throws ParameterError If parsing the query string failed.
   */
  public QueryParameterList(@Nullable CharSequence query_string, @Nullable QueryParameterDecoder decoder) throws ParameterError {
   ...
   * @throws ParameterError If parsing the query string failed.
   */
  public QueryParameterList(@Nullable CharSequence query_string, int start, int end, @Nullable QueryParameterDecoder decoder)
   ...
   * doing <pre>{@code return name.toLowerCase()}</pre>.
   *
  ...
  public @Nullable QueryParameter join(@NotNull String key) {
    ...
    final HashMap<@NotNull String, @NotNull QueryParameter> cache = joinedByKey != null ? joinedByKey : (joinedByKey = new HashMap<>());
    p = cache.get(key);
    if (p != null) {
      // Cache hit.
      return p;
