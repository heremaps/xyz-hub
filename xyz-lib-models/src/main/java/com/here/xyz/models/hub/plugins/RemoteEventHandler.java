...
/**
 * A remote event handler is an artificially deployed component of Naksha, which runs in an own process, accessed using a URL. All events
 * are “POST” (see {@link #method}) to the URL and wrapped into an {@link RemoteInvocation} object. The handler need to return either an
 * {@link Event} or an {@link XyzResponse}, which both extend {@link Payload}. If an {@link Event} is returned, this event is send upstream
 * in the event-pipeline. If an {@link XyzResponse} is returned, this response will be returned. Naksha provides everything that is needed
 * to create artificial extensions.
 */
...
public class RemoteEventHandler extends EventHandler {
  ...
  public @NotNull IEventHandler newInstance() throws Exception {
    try {
      ...
      //       The HTTP proxy will need the connector and this extension object, and will then, when it receives an event,
