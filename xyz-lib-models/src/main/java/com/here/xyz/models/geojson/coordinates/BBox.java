...
/**
 * A bounding box that follows the GeoJson specification, which states that: The value MUST be an array of length 2*n where n is the number
 * of dimensions represented in the contained geometries, with all axes of the most southwesterly point followed by all axes of the more
...
public class BBox extends ArrayList<Double> {
   ...
   *
   * @param shortestDistance If true, then the shortest distance is returned, that means when crossing the date border is shorter than the
   * other way around, this is returned. When false, then the date border is never crossed, what will result in bigger bounding boxes.
