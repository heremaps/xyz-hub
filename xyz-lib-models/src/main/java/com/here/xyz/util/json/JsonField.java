...
public abstract class JsonField<OBJECT, VALUE> implements CharSequence {
  ...
  public static final ConcurrentHashMap<Class, F5<JsonField, JsonClass, Field, Integer, String, String>> constructors = new ConcurrentHashMap<>();

  /**
   * Register a new JSON field type. Usage: <pre>{@code
   * static {
  ...
  public static <OBJECT, VALUE> void register(
      ...
      @NotNull F5<? extends JsonField<OBJECT, VALUE>, JsonClass<OBJECT>, Field, Integer, String, String> constructor
  ) {
  ...
  static @NotNull JsonField<?, ?> construct(@NotNull JsonClass<?> jsonClass, @NotNull Field javaField, int index) {
    ...
    if (valueClass.isPrimitive()) {
      ...
      throw new IllegalArgumentException("Invalid default value for field " + jsonClass.javaClass.getName() + "::" + javaField.getName());
    }
    //noinspection rawtypes
    final F5<JsonField, JsonClass, Field, Integer, String, String> constructor = constructors.get(jsonClass.javaClass);
    if (constructor != null) {
   ...
   */
  JsonField(@NotNull JsonClass<OBJECT> jsonClass, @NotNull Field javaField, int index, @NotNull String jsonName,
    ...
    this.fullName = StringCache.intern(
        jsonClass.javaClass.getName() + "::" + javaField.getName() + (jsonName != javaName ? "@{" + jsonName + "}" : "")
    );

    // Examples: "com.here.Foo::transaction" or "com.here.Foo::transaction@{tx}", if json-name differs.
   ...
   * @return the given value or an equivalent for {@code null}, in any case guaranteed to be a valid value for the field.
   * @throws IllegalArgumentException if the given value is of an invalid type.
   */
   ...
   * @throws ClassCastException if the access mode type matches the caller's symbolic type descriptor, but a reference cast fails.
   */
  public abstract void _put(@NotNull OBJECT object, VALUE value);

  /**
   * Sets the value of the field. When {@code expected} and {@code value} are the same, then still an CAS operation is done.
   *
   * @param object   the object to query.
   * @param expected the value that is expected.
   * @param value    the value to set.
   * @return {@code true} if the value was set; {@code false} if the current value is not the expected value.
   * @throws ClassCastException if the access mode type matches the caller's symbolic type descriptor, but a reference cast fails.
   */
