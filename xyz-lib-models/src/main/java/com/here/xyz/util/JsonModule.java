...
/**
 * Jackson JSON parser module that internalizes (de-duplicates) strings using the {@link StringCache}. This reduces the memory usage and
 * improves the performance when comparing strings, especially when strings as used in hash-maps as keys.
 */
public class JsonModule extends SimpleModule {
  ...
  private static final class StringKeySerializer extends StdSerializer<String> {
    ...
    @Override
    public void serialize(@NotNull String value, @NotNull JsonGenerator g, @NotNull SerializerProvider provider) throws IOException {
