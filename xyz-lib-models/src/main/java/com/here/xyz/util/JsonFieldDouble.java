...
public final class JsonFieldDouble<OBJECT> extends JsonField<OBJECT, Double> {
  ...
  JsonFieldDouble(@NotNull JsonClass<OBJECT> jsonClass, @NotNull Field javaField, int index, @NotNull String jsonName,
      @Nullable String defaultValue) {
    super(jsonClass, javaField, index, jsonName, defaultValue);
    this.nullValue = 0d;
    this.defaultValue = defaultValue != null && defaultValue.length() > 0 ? Double.parseDouble(defaultValue) : nullValue;
  }
  ...
  public boolean _compareAndSwap(@NotNull OBJECT object, Double expected, Double value) {
    ...
    return Unsafe.unsafe.compareAndSwapLong(object, offset, doubleToRawLongBits(expected), doubleToRawLongBits(value));
  }
