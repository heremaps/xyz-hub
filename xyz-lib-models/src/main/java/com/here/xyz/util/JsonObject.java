...
/**
 * A merge between a POJO and a hash-map, so fields are exposed next to arbitrary additional properties. In other words, this is a hash-map
 * that can be extended with native POJO members. In that case all Java fields annotated with {@link JsonProperty}, being {@code public} or
 * {@code protected} and <b>not</b> annotated with {@link JsonIgnore} are made available as hash-map key-value pairs. This is necessary for
 * the {@link Patcher} to work. Note that the map will reflect the properties by their annotated name, if annotated differently from the
 * name in the source code.
 *
 * <p><b>WARNING</b>: A problem arises when directly accessing additional properties and adding properties that have the same name as
 * the native fields. Modification of properties should only be done through the exposed Map interface or by directly accessing the native
 * properties, never using the {@link #additionalProperties() additional properties map}!</p>
 *
 * @since 2.0.0
 */
...
public class JsonObject implements Map<@NotNull String, @Nullable Object>, Iterable<Map.Entry<@NotNull String, @Nullable Object>> {
   ...
   *
   * <p><b>WARNUNG</b>: Do not use this to modify fields, this is mainly for Jackson to serialize the object! If the content of this map
   * is modified this can lead to severe problems, like duplicate fields with different content.</p>
   *
  ...
  /**
   * Assigns the given value to the given field. If the value {@link FibMap#UNDEFINED} is given, virtually removing the field.
   *
   * <p>Note that, due to the nature of Java fields, which can't be really removed, we emulate field removal and this is not really totally
   * atomic.
   *
   * @param field          the field to set.
   * @param expected_value the existing value that is expected for the operation to succeed; {@link FibMap#ANY} if any value is okay.
   * @param new_value      the value to set; if {@link FibMap#UNDEFINED} removing the field.
   * @param create         whether, if no such field exists yet, creating the field ({@code true} or {@code false}).
   * @return the previously assigned value, {@link FibMap#UNDEFINED}, if the field did not exist and {@link FibMap#CONFLICT} if the
   * operation failed due to a conflicting situation.
   * @throws NullPointerException     if any of the required parameters is {@code null}.
   * @throws IllegalArgumentException if the given array is empty (length of zero), the new value is {@link FibMap#CONFLICT} or
   *                                  {@link FibMap#ANY}, or the expected value is {@link FibMap#CONFLICT}.
   */
  ...
  static {
    try {
      REMOVED = MethodHandles.lookup().in(JsonObject.class).findVarHandle(JsonObject.class, "removed", long[].class);
