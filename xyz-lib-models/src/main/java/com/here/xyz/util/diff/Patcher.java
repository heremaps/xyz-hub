...
public class Patcher {
   ...
   * source state.
   */
  public static @Nullable Difference getDifference(final @Nullable Object sourceState, final @Nullable Object targetState) {
  ...
  private static MapDiff getMapDifference(
      final Map sourceState,
      final Map targetState,
      final @Nullable IgnoreKey ignoreKey
  ) {
   ...
   */
  private static ListDiff getListDifference(final List sourceList, final List targetList, final @Nullable IgnoreKey ignoreKey) {
  ...
  public static <T extends Difference> @NotNull Difference mergeDifferences(
      final @NotNull T diffA,
      final @NotNull T diffB,
      final @NotNull ConflictResolution cr
    ...
    if (diffA instanceof UpdateOp || diffA instanceof InsertOp) {
      ...
      if (valueA != valueB) {
        if (valueA == null || valueB == null || valueA.getClass() != valueB.getClass() || !valueA.equals(valueB)) {
          return switch (cr) {
            case ERROR -> throw new MergeConflictException("Conflict while merging " + diffA + " with " + diffB);
            case RETAIN -> diffA;
            case REPLACE -> diffB;
          };
        }
   ...
   */
  private static MapDiff mergeMapDifferences(MapDiff diffA, MapDiff diffB, ConflictResolution cr) throws MergeConflictException {
   ...
   */
  private static ListDiff mergeListDifferences(ListDiff diffA, ListDiff diffB, ConflictResolution cr) throws MergeConflictException {
   ...
   */
  private static void patchList(final List<Object> list, final ListDiff listDiff) throws NullPointerException, IllegalArgumentException {
    if (listDiff == null) {
      return;
    }
    try {
  ...
  public static @Nullable Difference calculateDifferenceOfPartialUpdate(
    ...
    for (final Object key : keys) {
      ...
      } else if (recursive && sourceObjectVal instanceof Map sourceObjectMap && partialUpdateVal instanceof Map partialUpdateMap) {
        final Difference childDiff = calculateDifferenceOfPartialUpdate(sourceObjectMap, partialUpdateMap, ignoreKey, true);
        if (childDiff != null) {
