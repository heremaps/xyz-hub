...
public final class JsonFieldFloat<OBJECT> extends JsonField<OBJECT, Float> {
  ...
  JsonFieldFloat(@NotNull JsonClass<OBJECT> jsonClass, @NotNull Field javaField, int index, @NotNull String jsonName,
      @Nullable String defaultValue) {
    super(jsonClass, javaField, index, jsonName, defaultValue);
    this.nullValue = 0f;
    this.defaultValue = defaultValue != null && defaultValue.length() > 0 ? Float.parseFloat(defaultValue) : nullValue;
  }
  ...
  public boolean _compareAndSwap(@NotNull OBJECT object, Float expected, Float value) {
    ...
    return Unsafe.unsafe.compareAndSwapInt(object, offset, Float.floatToRawIntBits(expected), Float.floatToRawIntBits(value));
  }
