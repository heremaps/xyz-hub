...
public final class IoHelp {
   ...
   * from the stream. If {@code len} is zero, then no bytes are read and an empty byte array is returned. Otherwise, up to {@code len} bytes
   * are read from the stream. Fewer than {@code len} bytes may be read if end of stream is encountered.
   *
   * <p> When this stream reaches end of stream, further invocations of this
   * method will return an empty byte array.
   *
   * <p> Note that this method is intended for simple cases where it is
   * convenient to read the specified number of bytes into a byte array. The total amount of memory allocated by this method is proportional
   * to the number of bytes read from the stream which is bounded by {@code len}. Therefore, the method may be safely called with very large
   * values of {@code len} provided sufficient memory is available.
   *
   * <p> The behavior for the case where the input stream is <i>asynchronously
   * closed</i>, or the thread interrupted during the read, is highly input stream specific, and therefore not specified.
   *
   * <p> If an I/O error occurs reading from the input stream, then it may do
   * so after some, but not all, bytes have been read. Consequently the input stream may not be at end of stream and may be in an
   ...
   */
  public static <T> @NotNull T parseValue(@Nullable Object value, @NotNull Class<T> type, @NotNull T defaultValue,
   ...
   * @throws IllegalArgumentException If {@code ignoreInvalid} if {@code false} and the value does not match the expected type.
   */
  public static <T> @Nullable T parseNullableValue(@Nullable Object value, @NotNull Class<T> type, @Nullable T defaultValue,
      boolean ignoreInvalid) {
    if (value == null) {
      return defaultValue;
    }
