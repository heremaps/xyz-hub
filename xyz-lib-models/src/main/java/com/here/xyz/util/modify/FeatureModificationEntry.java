...
public class FeatureModificationEntry<FEATURE extends Feature> {
  ...
  final @Nullable Action apply() throws ModificationException, MergeConflictException {
    ...
    assert false;
    final var e = new ModificationException("Reached unexpected step while processing feature {" + input.getId() + "}");
  ...
  private @Nullable FEATURE merge() throws MergeConflictException {
    ...
    if (base == null
        || base == head
        || Objects.equals(base.getProperties().getXyzNamespace().getUuid(), input.getProperties().getXyzNamespace().getUuid())) {
      return input;
    }
    ...
    if (baseToHeadDiff == null) {
      ...
      // Eventually this means, that we can just treat the input as a direct modification of the head.
      input.getProperties().getXyzNamespace().setUuid(head.getProperties().getXyzNamespace().getUuid());
      return input;
    }
    ...
    if (baseToInputDiff == null) {
      // Eventually nothing changed, so basically the changes effectively resulting in the same head state.
      return null;
    }
