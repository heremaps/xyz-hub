 ...
 * the good and the bad. In Naksha we use {@link String}'s as keys, for which the Fibonacci distribution seems to fit perfectly well.
 *
 * <p>Beware that we only use Fibonacci as distribution, this map does not implement an own hash algorithm, it still operates best with
...
public final class FibMap {
  ...
  //
  // This does not apply to ARM. For ARM CPU we add the LoadLoad barriers, which will effectively translate into a LoadLoadStore barrier:
  // * Load - Load/Store: This means that the barrier requires all loads to complete before the barrier but does not require
  //                      stores to complete. Both loads and stores that appear after the barrier in program order must wait
  ...
  /**
   * The 32-bit hash multiplier ({@code 2654435769}). This was calculated using
   * <a href="https://keisan.casio.com/calculator">keisan.casio.com</a> and then converted into binary using <a
   ...
   * amount of 16^8 (4,294,967,296, ~4 billion) distinct keys, when combined with the default {@link #SEGMENT_bits}. If that level reached,
  ...
  /**
   * Returns the index between {@code 0 and 2^bits-1} using <a href="https://news.ycombinator.com/item?id=17328756">Fibonacci Hashing</a>.
   * This guarantees that the lowest bit is always zero.
   *
   * @param hash  The hash for which to calculate the index.
   * @param bits  The amount of bits to use (the maximum value will then be 2^bits-1).
   * @param depth The depth, a value greater than or equal to zero, where zero means (root level).
  ...
  public static int keyValuePairsOf(Object @NotNull [] array) {
    ...
    // For an array with 1 bit, two values, therefore one key-value pair: (1 << 1) >>> 1 == (2) >>> 1 == 1.
    // For an array with 2 bits, four values, therefore two key-value pairs: (1 << 2) >>> 1 == (4) >>> 1 == 2.
    // ...
    return (1 << capacityBitsOf(array)) >>> 1;
  }

  /**
   * Returns the next entry that stores the given value or {@code null} if no such value found. Note that the method accepts an entry as
   * input, which will be returned when a result found. This makes the call garbage free, no matter if a value found or not.
   *
   * @param startKey The key to start search (ignoring the value of this key and any value before it); {@code null} if start at the first
   *                 available key.
  ...
  /**
   * Returns the next entry that stores the given value, {@code null} if no such value found or {@link #START_KEY_IS_NULL}, if the
   * {@code startKey} was found, but no further occurrence of the value.
   *
   * @param startKey The key to start search (ignoring the value of this key and any value before it); {@code null} if start at the first
   *                 available key.
   * @param value    The value to search.
   * @param array    The key-value array in which to search.
   * @param entry    The entry to return; if {@code null}, creating a new entry on demand.
   * @return The next key-value entry, {@code null} if no such value found or {@link #START_KEY_IS_NULL}, if the {@code startKey} was found,
   * but no further occurrence of the value.
   */
  ...
  /**
   * The default implementation of interning keys. This method only interns {@link String}'s using the {@link StringCache}.
   *
  ...
  /**
   * Assigns the given key to the given value. If the value {@link #UNDEFINED} is given, removing the key. This method uses the default
   * values for segment sizes, key interning and maximum depth.
   *
   * @param key            The key to look for.
   * @param expected_value the existing value that is expected for the operation to succeed; {@link #ANY} if any value is okay or
   *                       {@link #VOID} if {@link #UNDEFINED} and {@code null} are acceptable.
   * @param new_value      The value to set; if {@link #UNDEFINED} removing the key.
   * @param create         Whether, if no such key exists yet, creating the key ({@code true} or {@code false}), in doubt use {@code true}.
   * @param array          The array in which to search.
   * @return The previously assigned value, {@link #UNDEFINED}, if the key did not exist and {@link FibMapConflict} if the operation failed
   * due to a conflicting situation.
   * @throws NullPointerException     If any of the required parameters is {@code null}.
   * @throws IllegalArgumentException If the given array is empty (length of zero), the new value is {@link FibMapConflict} or {@link #ANY},
   *                                  or the expected value is {@link FibMapConflict}.
   */
  ...
  /**
   * Assigns the given key to the given value. If the value {@link #UNDEFINED} is given, removing the key.
   *
   * @param key            The key to look for.
   * @param expected_value the existing value that is expected for the operation to succeed; {@link #ANY} if any value is okay or
   *                       {@link #VOID} if {@link #UNDEFINED} and {@code null} are acceptable.
   ...
   * @param conflict       the method to create a conflict case, in doubt use {@link #conflict(Object, Object, Object, Object)}.
   * @return The previously assigned value, {@link #UNDEFINED}, if the key did not exist, and some arbitrary conflict case, if the operation
   * failed due to a conflicting situation.
   * @throws NullPointerException     If any of the required parameters is {@code null}.
   * @throws IllegalArgumentException If the given array is empty (length of zero), the new value is {@link FibMapConflict} or {@link #ANY},
   *                                  or the expected value is {@link FibMapConflict}.
   */
  public static @Nullable Object put(
      ...
      final @NotNull F4<@NotNull Object, @NotNull Object, @Nullable Object, @Nullable Object, @Nullable Object> conflict
  ) {
    if (array.length == 0) {
  ...
  static @Nullable Object _put(
    ...
    while (true) {
        ...
        if (segment.length > LPT_HEADER_SIZE && segment[LPT_ID] == LPT_ID_OBJECT) {
          ...
          if (ARRAY.compareAndSet(array, ki, segment, LOCK_OBJECT)) {
            ...
            try {
              ...
              while (j < segment.length) {
                ...
                if (the_key != null && ILike.equals(the_key, key)) {
                  ...
                  if (new_value == UNDEFINED) {
                    // We want the key to become visible before the value, because for null-keys the value is ignored.
                    segment[j] = null;
                    storeStoreFence();
                    segment[j + 1] = null;
                  } else {
              ...
              // We ensure order:
              // Write value first, when this becomes visible to readers before the key-write, they will ignore it, because key is null.
              storeStoreFence();
              segment[empty] = key;
              return UNDEFINED;
            } finally {
      ...
      if (existing_key != null) {
        ...
        // This code is a duplicate, but checking right here safes use from allocating memory for the sub-array, if not needed.
        if (new_value == UNDEFINED) {
