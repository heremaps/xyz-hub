...
public class MainHealthCheck extends GroupedHealthCheck {
  ...
  public final void initialize() {
    if (isGracePeriodActivated()) {
      ...
      long remainingGraceTime = Config.getInt(BOOT_GRACE_TIME)
      ...
      int maxTimeout = checks.isEmpty()
          ? 0
          : checks.stream()
              .mapToInt(ExecutableCheck::getTimeout)
              .max()
              .getAsInt();
  ...
  public final Response getResponse() {
     ...
     */
    Status s = execute().withTimestamp(CachedClock.instance().currentTimeMillis());
    setStatus(s);
