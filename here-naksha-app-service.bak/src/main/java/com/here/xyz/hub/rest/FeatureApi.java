...
public class FeatureApi extends SpaceBasedApi {
  ...
  private void putFeature(final RoutingContext context) {
    ...
    executeConditionalOperationChain(
        false,
        context,
        ApiResponseType.FEATURE,
        IfExists.REPLACE,
        IfNotExists.CREATE,
        true,
  ...
  private void putFeatures(final RoutingContext context) {
    executeConditionalOperationChain(
        false,
        context,
        getEmptyResponseTypeOr(context, ApiResponseType.FEATURE_COLLECTION),
        IfExists.REPLACE,
        IfNotExists.CREATE,
        true,
        ConflictResolution.ERROR);
  ...
  private void postFeatures(final RoutingContext context) {
    ...
    final ConflictResolution conflictResolution =
        ConflictResolution.of(Query.getString(context, Query.CONFLICT_RESOLUTION, "error"));
  ...
  private void deleteFeature(final RoutingContext context) {
    Map<String, Object> featureModification =
        Collections.singletonMap("featureIds", Collections.singletonList(context.pathParam(Path.FEATURE_ID)));
  ...
  private void deleteFeatures(final RoutingContext context) {
    ...
    if (featureIds != null && !featureIds.isEmpty()) {
      Map<String, Object> featureModification =
          Collections.singletonMap("featureIds", new ArrayList<>(featureIds));
    ...
    // Delete features by tags
      ...
      if (checkModificationOnSuper(context, spaceContext)) return;
      ...
      context.fail(new HttpException(
          HttpResponseStatus.BAD_REQUEST,
          "At least one identifier should be provided as a query parameter."));
    }
  }
   ...
   */
  private void executeConditionalOperationChain(
    ...
    if (checkModificationOnSuper(context, getSpaceContext(context))) return;
    executeConditionalOperationChain(
  ...
  private void executeConditionalOperationChain(
    ...
    ModifyFeaturesEvent event =
        new ModifyFeaturesEvent().withTransaction(transactional).withContext(spaceContext);
    event.setActionIfRowLocked(
        IfRowLock.of(Query.getString(context, Query.IF_ROW_LOCKED, IfRowLock.WAIT.toString())));
    int bodySize = context.getBody() != null ? context.getBody().length() : 0;
    ConditionalModifyFeaturesTask task = buildConditionalOperation(
        event,
        context,
        apiResponseTypeType,
  ...
  private ConditionalModifyFeaturesTask buildConditionalOperation(
    ...
    if (featureModifications == null)
      return new ConditionalModifyFeaturesTask(
          event,
          context,
          apiResponseTypeType,
          ifNotExists,
    ...
    final XyzModificationList modifyFeatureOp =
        new XyzModificationList(featureModifications, ifNotExists, ifExists, transactional, cr);
    return new ConditionalModifyFeaturesTask(
        event, context, apiResponseTypeType, modifyFeatureOp, requireResourceExists, bodySize);
  }
}
