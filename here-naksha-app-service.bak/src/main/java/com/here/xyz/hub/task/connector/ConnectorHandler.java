...
public class ConnectorHandler {
  ...
  public static void getConnector(
      RoutingContext context, String connectorId, Handler<AsyncResult<Connector>> handler) {
    ...
    Service.connectorConfigClient.get(marker, connectorId, ar -> {
      if (ar.failed()) {
        ...
        handler.handle(Future.failedFuture(
            new HttpException(NOT_FOUND, "The requested resource does not exist.", ar.cause())));
      } else {
  ...
  public static void getConnectors(
      RoutingContext context, List<String> connectorIds, Handler<AsyncResult<List<Connector>>> handler) {
    Marker marker = Context.getMarker(context);
    ...
    connectorIds.forEach(connectorId -> {
      ...
      Service.connectorConfigClient.get(marker, connectorId, ar -> {
        if (ar.failed()) {
          logger.warn(marker, "The requested resource does not exist.'", ar.cause());
          f.completeExceptionally(new HttpException(
              NOT_FOUND, "The requested resource '" + connectorId + "' does not exist.", ar.cause()));
        } else {
    ...
    CompletableFuture.allOf(completableFutureList.toArray(new CompletableFuture[0]))
        ...
        .thenRun(() -> handler.handle(Future.succeededFuture(completableFutureList.stream()
            .map(CompletableFuture::join)
            .collect(Collectors.toList()))));
  }

  public static void getConnectors(
      RoutingContext context, String ownerId, Handler<AsyncResult<List<Connector>>> handler) {
    Marker marker = Context.getMarker(context);
    ...
    Service.connectorConfigClient.getByOwner(marker, ownerId, ar -> {
      if (ar.failed()) {
        ...
        handler.handle(Future.failedFuture(new HttpException(
            INTERNAL_SERVER_ERROR, "Unable to load the resource definitions.", ar.cause())));
      } else {
        handler.handle(Future.succeededFuture(ar.result()));
      }
    });
  ...
  public static void createConnector(
      RoutingContext context, JsonObject connector, Handler<AsyncResult<Connector>> handler) {
    Marker marker = Context.getMarker(context);
    ...
    Service.connectorConfigClient.get(marker, connector.getString("id"), ar -> {
      if (ar.failed()) {
        ...
        handler.handle(Future.failedFuture(
            new HttpException(BAD_REQUEST, "Resource with the given ID already exists.")));
      }
    });
  }

  protected static void storeConnector(
      RoutingContext context,
      JsonObject connector,
      Handler<AsyncResult<Connector>> handler,
      Marker marker,
  ...
  private static void storeConnector(
      RoutingContext context,
      Handler<AsyncResult<Connector>> handler,
      Marker marker,
      AsyncResult<Connector> ar,
    ...
    Service.connectorConfigClient.store(marker, c, ar2 -> {
      if (ar2.failed()) {
        ...
        handler.handle(Future.failedFuture(new HttpException(
            INTERNAL_SERVER_ERROR, "Unable to store the resource definition.", ar2.cause())));
      } else {
  ...
  public static void updateConnector(
    ...
    Service.connectorConfigClient.get(marker, connector.getString("id"), ar -> {
      if (ar.failed()) {
        ...
        handler.handle(Future.failedFuture(
            new HttpException(NOT_FOUND, "Unable to load the resource definition.", ar.cause())));
      } else {

        Connector oldConnector = ar.result();
        Map oldConnectorMap = asMap(oldConnector);
        MapDiff diffMap = (MapDiff)
            Patcher.calculateDifferenceOfPartialUpdate(oldConnectorMap, asMap(connector), null, true);
        try {
          if (diffMap == null) {
      ...
      if (arGet.succeeded()) {
        Service.connectorConfigClient.delete(marker, connectorId, ar -> {
          if (ar.failed()) {
            ...
            handler.handle(Future.failedFuture(new HttpException(
                INTERNAL_SERVER_ERROR, "Unable to delete the resource definition.", ar.cause())));
          } else {
            handler.handle(Future.succeededFuture(ar.result()));
          }
        });
        ...
        handler.handle(Future.failedFuture(
            new HttpException(NOT_FOUND, "The requested resource '" + connectorId + "' does not exist.")));
      }
    });
  }
  ...
  private static void validate(RoutingContext context, Connector connector) throws HttpException {
    ...
    if (connector.id == null) throw new HttpException(BAD_REQUEST, "Parameter 'id' for the resource is missing.");
    else if (connector.id.length() < ID_MIN_LENGTH || connector.id.length() > ID_MAX_LENGTH)
      throw new HttpException(
          BAD_REQUEST,
          "Parameter 'id' must have a minimum length of  " + ID_MIN_LENGTH + " and a " + "maximum length of "
              + ID_MAX_LENGTH);
    ...
    if (connector.owner == null) connector.owner = Context.jwt(context).aid;
  ...
  private static void validateAWSLambda(Connector connector, AWSLambda rf) throws HttpException {
    ...
    else if (!rf.lambdaARN.matches(
        "arn:aws:lambda:[a-z]{2}(-gov)?-[a-z]+-\\d{1}:\\d{12}:function:([a-zA-Z0-9-_]+)(:(\\$LATEST|[a-zA-Z0-9-_]+))?"))
      throw new HttpException(
          BAD_REQUEST, "Parameter 'remoteFunction.lambdaARN' must be a valid Lambda function ARN.");
  ...
  private static void validateHttp(Connector connector, Http rf) throws HttpException {
    ...
    if (rf.warmUp != 0)
      throw new HttpException(
          BAD_REQUEST, "Parameter 'remoteFunction.warmup' must not be set for HTTP connectors.");
  ...
  private static void validateRfId(RemoteFunctionConfig rf) throws HttpException {
    if (rf.id == null)
      ...
      throw new HttpException(
          BAD_REQUEST,
          "Parameter 'remoteFunction.id' must have a minimum length of  " + ID_MIN_LENGTH + " and a "
  ...
  private static void validateEmbedded(Connector connector, Embedded rf) throws HttpException {
    ...
    if (rf.warmUp != 0)
      throw new HttpException(
          BAD_REQUEST, "Parameter 'remoteFunction.warmup' must not be set for Embedded connectors.");
    ...
    try {
      ...
      if (!AbstractConnectorHandler.class.isAssignableFrom(connectorClass))
        throw new HttpException(
            BAD_REQUEST,
            "Parameter 'remoteFunction.className' can't be resolved to a valid connector handler class.");
  ...
  private static void validateConnectionSettings(Connector connector, int maxConnections) throws HttpException {
    // Validate ConnectionSettings
    if (connector.connectionSettings.getMinConnections() < 0
        || connector.connectionSettings.getMinConnections() > 256)
      throw new HttpException(
          BAD_REQUEST, "Parameter 'connectionSettings.minConnections' needs to be in the range of 0-256.");
    if (connector.connectionSettings.maxConnections < 16
        || connector.connectionSettings.maxConnections > maxConnections)
      throw new HttpException(
          BAD_REQUEST,
          "Parameter 'connectionSettings.maxConnections' needs to be in the range of 16-" + maxConnections
              + ".");
  }

  private static void validateMaxPayload(Connector connector, int maxPayloadSize) throws HttpException {
    // Validate Capabilities
    if (connector.capabilities.maxPayloadSize < 0
        || connector.capabilities.maxPayloadSize > maxPayloadSize * 1024 * 1024)
      throw new HttpException(
          BAD_REQUEST,
          "Parameter 'capabilities.maxPayloadSize' needs to be in the range of 0-"
              + (maxPayloadSize * 1024 * 1024) + " (default: " + (maxPayloadSize * 1024 * 1024) + ").");
  }
}
