...
import com.here.naksha.lib.core.util.JsonUtils;
...
public abstract class NakshaTask<EVENT extends Event> extends AbstractTask<EVENT> {
  ...
  private static @NotNull ApiResponseType extractResponseType(
      @NotNull RoutingContext routingContext, @NotNull ApiResponseType... allowedTypes) {
  ...
  public static <E extends Event> void start(
    ...
    try {
      ...
      NakshaRoutingContext.sendFatalErrorResponse(
          routingContext, "Failed to create task: " + eventClass.getSimpleName());
    }
  }
  ...
  protected void initEventFromRoutingContext(
    ...
    if (ifNoneMatch != null) {
      ...
      if (matcher.matches()) {
        ...
        logger().info(
                "The externally provided 'If-None-Match' contains illegal characters, ignoring it: {}",
                ifNoneMatch);
      }
    }
    ...
    if (!routingContext.body().isEmpty()) {
      ...
      if (buffer.length() > 0) {
        try {
          ...
          // Note: Release the memory allocated for the request, vertx does not reuse the buffer and does not
          // use pooled buffers!
   ...
   */
  protected void handleBodyBuffer(final @NotNull RoutingContext routingContext, final Buffer buffer)
      throws Exception {}
   ...
   * @return the response.
   */
  protected final @NotNull XyzResponse sendAuthorizedEvent(
      @NotNull Event event, @NotNull XyzHubActionMatrix eventMatrix) {
    ...
    if (rightsMatrix == null || !rightsMatrix.matches(eventMatrix)) {
      final String errorMessage = "Insufficient rights. " + "\nToken access: "
          + Json.encode(rightsMatrix) + "\nRequest access: "
          + Json.encode(eventMatrix);
