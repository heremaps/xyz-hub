...
public class SubscriptionApi extends Api {
  ...
  public static class SubscriptionAuthorization extends Authorization {
    ...
    public static void authorizeManageSpacesRights(
        RoutingContext context, String spaceId, Handler<AsyncResult<Void>> handler) {
      final XyzHubActionMatrix requestRights =
          new XyzHubActionMatrix().manageSpaces(new XyzHubAttributeMap().withValue(SPACE, spaceId));
      try {
        evaluateRights(
