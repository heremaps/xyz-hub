  ...
  public Status execute() throws InterruptedException {
    ...
    try {
      if (cachedStatus != null) {
        ...
        if (tmpStatus.getResult().compareTo(UNAVAILABLE) >= 0) consecutiveFailures++;
          ...
          try {
            if (ar.failed()) {
              setResponse(generateResponse()
                  .withMessage("Error in connector health-check: "
                      + ar.cause().getMessage()));
              s.setResult(ERROR);
              consecutiveFailures++;
            } else {
      ...
      setResponse(
          generateResponse().withMessage("Error trying to execute health-check event: " + e.getMessage()));
      cancelHcRequest(hcHandle);
