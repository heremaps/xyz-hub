...
public class RemoteFunctionHealthAggregator extends GroupedHealthCheck {
  ...
  public Status execute() {
    ...
    if (cachedResponse != null
      ...
      cachedResponse.getChecks().forEach(c -> {
        ...
        if (check != null) check.injectCachedResponse(c.getStatus(), c.getResponse());
      });
    } else {
      ...
      try {
        List<Connector> activeConnectors = RemoteFunctionClient.getInstances().stream()
            .map(RemoteFunctionClient::getConnectorConfig)
            .collect(Collectors.toList());

        Set<String> activeConnectorIds =
            activeConnectors.stream().map(c -> c.id).collect(Collectors.toSet());
        ...
        res = res.withMessage("Error when trying to gather remote functions info: ("
      ...
      CacheClient.getInstance()
          .set(
              RFC_HC_CACHE_KEY,
              res.toInternalResponseString().getBytes(),
    ...
    try {
      ...
      res.setAdditionalProperty(
          "globalUsedConnectionsPercentage", RemoteFunctionClient.getGlobalUsedConnectionsPercentage());
      setResponse(res);
  ...
  private Response fetchResponseFromCache() {
    ...
    CacheClient.getInstance().get(RFC_HC_CACHE_KEY).onSuccess(r -> {
      ...
      try {
        if (r != null) response = new JsonObject(Buffer.buffer(r)).mapTo(Response.class);
