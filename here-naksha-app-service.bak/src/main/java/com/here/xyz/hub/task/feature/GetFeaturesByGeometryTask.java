    ...
    try {
      ...
      if (input instanceof Geometry) {
        ...
        throw new ParameterError(
            "The provided content is of type '"
      ...
      throw new ParameterError(
          "Invalid JSON string. Error at line " + e.getLocation().getLineNr() + ", column "
  ...
  protected @NotNull XyzResponse execute() throws Exception {
    if (refSpace != null) {
      if (!refSpace.getId().equals(getSpace().getId())) {
        // TODO: Implement this by creating a new task that loads the referred feature (refFeatureId) and then
        // uses the geometry of
