...
public class OpenApiGenerator {
  ...
  private static void validateRecipe() throws Exception {
    if (!recipe.has(RECIPES)
        || !recipe.get(RECIPES).isArray()
        || recipe.get(RECIPES).size() == 0) {
  ...
  private static void validateInheritance() throws Exception {
    ...
    for (JsonNode recipe : recipes) {
      if (recipe.has(EXTENDS)) {
        if (!recipe.get(EXTENDS).isTextual()
            || !RECIPES_MAP
                .keySet()
                .containsAll(Arrays.asList(
                    recipe.get(EXTENDS).textValue().split(",")))) {
          throw new Exception(
              "Invalid recipe. Recipe's field \"extends\" must reference a valid recipe within the recipes' names list.");
        }
      }
    }
  }
  ...
  private static void prepareRecipe(final String name) {
    ...
    if (recipe.has(EXTENDS)) {
      final List<String> extensions =
          Arrays.asList(recipe.get(EXTENDS).textValue().split(","));
      Collections.reverse(extensions);

      for (String extension : extensions) {
        final JsonNode parent = RECIPES_MAP.get(extension);
        elements(parent).entrySet().stream()
  ...
  private static void processStarReplacement(JsonNode find, JsonNode replace) {
    ...
    traverse(root, node -> elements(node).forEach((k, v) -> {
  ...
  public static void main(String... args) {
    try {
      ...
      final byte[] result =
          useName ? generate(sourceBytes, recipeBytes, args[i]) : generate(sourceBytes, recipeBytes);
