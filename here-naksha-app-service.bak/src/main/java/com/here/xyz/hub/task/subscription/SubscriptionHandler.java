...
public class SubscriptionHandler {
  ...
  public static void getSubscription(
      ...
      io.vertx.core.Handler<AsyncResult<Subscription>> handler) {
    ...
    Service.subscriptionConfigClient.get(marker, subscriptionId).onComplete(ar -> {
      if (ar.failed()) {
        ...
        if (!spaceId.equals(subscription.getSource())) {
          logger.warn(
              marker,
              "The requested source {} does not match the source {} of the subscription {}",
              spaceId,
  ...
  public static void getSubscriptions(
      RoutingContext context, String source, io.vertx.core.Handler<AsyncResult<List<Subscription>>> handler) {
    Marker marker = Context.getMarker(context);
  ...
  public static void getAllSubscriptions(
      RoutingContext context, io.vertx.core.Handler<AsyncResult<List<Subscription>>> handler) {
    Marker marker = Context.getMarker(context);
  ...
  public static void createSubscription(
    ...
    Service.subscriptionConfigClient.get(marker, subscription.getId()).onComplete(ar -> {
      if (ar.failed()) {
        ...
        handler.handle(
            Future.failedFuture(new HttpException(CONFLICT, "Resource with the given ID already exists.")));
      }
    });
  }

  public static void createOrReplaceSubscription(
      RoutingContext context,
      Subscription subscription,
      io.vertx.core.Handler<AsyncResult<Subscription>> handler) {
    Marker marker = Context.getMarker(context);
  ...
  protected static void storeSubscription(
      RoutingContext context,
      Subscription subscription,
      io.vertx.core.Handler<AsyncResult<Subscription>> handler,
      Marker marker) {
    ...
    Service.subscriptionConfigClient.store(marker, subscription).onComplete(ar -> {
      if (ar.failed()) {
        ...
        handler.handle(Future.failedFuture(new HttpException(
            INTERNAL_SERVER_ERROR, "Unable to store the resource definition.", ar.cause())));
      } else {
        handler.handle(Future.succeededFuture(subscription));
      }
    });
    ...
    getSubscriptions(context, subscription.getSource(), ar -> {
      if (ar.failed()) {
        ...
        sendEvent(context, Operation.DELETE, subscription, !hasActiveSubscriptions, marker, eventAr -> {
          if (eventAr.failed()) {
            if (eventAr.cause() instanceof HttpException
                && ((HttpException) eventAr.cause()).status.equals(NOT_FOUND)) {
  ...
  protected static void removeSubscription(
      RoutingContext context,
      Subscription subscription,
      io.vertx.core.Handler<AsyncResult<Subscription>> handler) {
    Marker marker = Context.getMarker(context);
  ...
  private static void sendEvent(
      ...
      Operation op,
      Subscription subscription,
      boolean hasNoActiveSubscriptions,
      Marker marker,
      io.vertx.core.Handler<AsyncResult<Subscription>> handler) {
