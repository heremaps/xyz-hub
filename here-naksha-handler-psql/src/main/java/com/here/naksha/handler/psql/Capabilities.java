...
public class Capabilities {
    ...
    private static List<String> sortableCanSearchForIndex(List<String> indices) {
        ...
        for (String k : indices) if (k.startsWith("o:")) skeys.add(k.replaceFirst("^o:([^,]+).*$", "$1"));
    ...
    public static class IndexList {
        ...
        public static @Nullable List<@NotNull String> getIndexList(@NotNull PsqlHandler psqlConnector)
            ...
            indexList = psqlConnector.executeQuery(
                    SQLQueryBuilder.generateIDXStatusQuery(psqlConnector.spaceSchema(), psqlConnector.spaceTable()),
    ...
    public static IndexList rsHandler(ResultSet rs) {
        try {
            ...
            for (Map<String, Object> one : raw) {
                 ...
                 */
                if (one.get("src").equals("a") || one.get("src").equals("m")) indices.add((String) one.get("property"));
