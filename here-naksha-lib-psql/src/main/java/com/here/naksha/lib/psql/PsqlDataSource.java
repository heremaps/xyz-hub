...
public class PsqlDataSource implements DataSource {
   ...
   * the search path, and the correct role pre-selected. Note that these settings can be modified by overriding the
   * {@link #initConnection(Connection)} and/or {@link #initSession(StringBuilder)} methods.
   *
   ...
   * later aborted, the effects of the SET command disappear when the transaction is rolled back. Once the surrounding transaction is
   ...
   * documentation</a>. Therefore, the query that is created in this method is committed, because we do not know how many transactions are
   * done with the connection and we want all of them to use the same settings.
   *
   * @param sb The string builder in which to create the query.
   * @throws SQLException If any error occurred.
   */
  protected void initSession(@NotNull StringBuilder sb) {
