...
public class WebMercatorTile {
   ...
   * @return The level of detail if it is a value between 1 (lowest detail) and (including) 23 (highest detail).
   ...
   * @param levelOfDetail Level of detail, from 1 (lowest detail) to 23 (highest detail).
   * @param pos If given, then the latitude and longitude values are stored in that object and the object is returned, if null, a new
   * position instance is created.
  ...
  public static WebMercatorTile getTileFromLatLonLev(double lat, double lon, int lev) {
    double sinLatitude = Math.sin(lat * Math.PI / 180),
        ...
        col = Math.floor((.5 - Math.log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI)) * Math.pow(2, lev));
   ...
   * @throws IllegalArgumentException If the given quadKey argument contains an invalid digit or is too short/long.
   */
