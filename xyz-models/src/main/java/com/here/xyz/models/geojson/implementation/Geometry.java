...
public abstract class Geometry implements Typed {
  ...
  /**
   * A helper method that validates the given object as GeoJSON Point coordinates. When the method returns, raw is guaranteed to be a
   * List&lt;Number&gt;.
   *
  ...
  public static void validatePointCoordinates(final Object raw) throws InvalidGeometryException {
    ...
    @SuppressWarnings("unchecked") final List<Object> values = (List<Object>) raw;
    if (values.size() < 2) {
    ...
    if (!(rawLon instanceof Number)) {
      throw new InvalidGeometryException("The longitude (1st) value in coordinates of the Point is no valid number");
    }
    if (!(rawLat instanceof Number)) {
      throw new InvalidGeometryException("The latitude (2nd) value in coordinates of the Point is no valid number");
    }
    final double lon = ((Number) rawLon).doubleValue();
    final double lat = ((Number) rawLat).doubleValue();
    if (lon < -180d || lon > +180d) {
      throw new InvalidGeometryException("The longitude (1st) value in coordinates of the Point is out of bounds (< -180 or > +180)");
    }
    if (lat < -90d || lat > +90d) {
      throw new InvalidGeometryException("The latitude (2nd) value in coordinates of the Point is out of bounds (< -90 or > +90)");
    }
    if (values.size() == 3) {
      final Object rawAlt = values.get(2);
      if (!(rawAlt instanceof Number)) {
        throw new InvalidGeometryException("The altitude (3rst) value in coordinates of the Point is no valid number");
      }
    }
  }
  ...
  public static void validateMultiPointCoordinates(final Object raw) throws InvalidGeometryException {
    ...
    @SuppressWarnings("unchecked") final List<Object> points = (List<Object>) raw;
   ...
   *
   * @param raw the object to test.
   * @throws InvalidGeometryException if the given object does not reflect MultiLineString coordinates.
   */
  public static void validateMultiLineStringCoordinates(final Object raw) throws InvalidGeometryException {
    if (!(raw instanceof List)) {
      throw new InvalidGeometryException("Coordinates are expected to be an array");
    }
    @SuppressWarnings("unchecked") final List<Object> lineStrings = (List<Object>) raw;
    for (int i = 0; i < lineStrings.size(); i++) {
      final Object lineString = lineStrings.get(i);
      if (!(lineString instanceof List)) {
        throw new InvalidGeometryException("Expected LineString at index #" + i + " of the MultiLineString coordinates");
      }
   ...
   *
   * @param raw the object to test.
   * @param isExterior if the ring is the exterior ring (the first element in the parent array); when false then this is an interior ring.
   * @throws InvalidGeometryException if the given object does not reflect LinearRing coordinates.
   */
  public static void validateLinearRingCoordinates(final Object raw, boolean isExterior) throws InvalidGeometryException {
    if (!(raw instanceof List)) {
      throw new InvalidGeometryException("Coordinates are expected to be an array");
    }
    ...
    //
    // Note: A linear ring MUST follow the right-hand rule with respect to the area it bounds, i.e., exterior rings are
    // counterclockwise, and holes are clockwise.
    //
    // But: Note: the [GJ2008] specification did not discuss linear ring winding order. For backwards compatibility, parsers SHOULD NOT
    ...
    if (firstPoint.size() != lastPoint.size() || //
        ...
        (firstPoint.size() == 3 && firstPoint.get(2).doubleValue() != lastPoint.get(2).doubleValue())) {
      throw new InvalidGeometryException("Expected first and last point to be the same in the LinearRing coordinates");
    }
  }
  ...
  public static void validatePolygonCoordinates(final Object raw) throws InvalidGeometryException {
    ...
    for (int i = 0; i < linearRings.size(); i++) {
      ...
      if (!(linearRing instanceof List)) {
        throw new InvalidGeometryException("Expected LinearRing at index #" + i + " of the Polygon coordinates");
      }
  ...
  public static void validateMultiPolygonCoordinates(final Object raw) throws InvalidGeometryException {
    ...
    for (int i = 0; i < polygons.size(); i++) {
      ...
      if (!(polygon instanceof List)) {
        throw new InvalidGeometryException("Expected Polygon at index #" + i + " of the MultiPolygon coordinates");
      }
