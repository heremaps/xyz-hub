...
public final class PsqlPoolConfig {
   ...
   * <p>
   * The timeout is measured from the time a command arrives at the server until it is completed by the server. If multiple SQL statements
   * appear in a single simple-Query message, the timeout is applied to each statement separately. (PostgreSQL versions before 13 usually
   * treated the timeout as applying to the whole query string). In extended query protocol, the timeout starts running when any
   * query-related message (Parse, Bind, Execute, Describe) arrives, and it is canceled by completion of an Execute or Sync message.
   */
  @JsonProperty
   ...
   * <p>
   * Unlike statement_timeout, this timeout can only occur while waiting for locks. Note that if statement_timeout is nonzero, it is rather
   * pointless to set lock_timeout to the same or larger value, since the statement timeout would always trigger first. If
  ...
  /**
   * This property controls the maximum amount of time (in milliseconds) that a connection is allowed to sit idle in the pool. Whether a
   * connection is retired as idle or not is subject to a maximum variation of +30 seconds, and average variation of +15 seconds. A
   * connection will never be retired as idle before this timeout. A value of 0 means that idle connections are never removed from the
   * pool.
   */
  @JsonProperty
  ...
  PsqlPoolConfig(
    ...
    this.maxPoolSize = maxPoolSize != null && maxPoolSize >= this.minPoolSize ? maxPoolSize : max(this.minPoolSize, 200);
