...
public class MaintenanceSQL {
  ...
  public static String generateCheckSchemasAndIdxTableSQL(String schema) {
    return "SELECT array_agg(nspname) @> ARRAY['" + schema + "'] as main_schema, "
        ...
        + "(SELECT (to_regclass('" + XYZ_CONFIG_SCHEMA + "." + XYZ_CONFIG_TXN_PUB_TABLE + "') IS NOT NULL) as txn_pub_table), "
        + "FROM( "
  ...
  /**
   * Check if the xyz_ext_version() function is installed, which is used for versioning the sql-functions.
   */
  ...
  public static String configSchemaAndSystemTablesSQL =
      "CREATE SCHEMA IF NOT EXISTS \"" + XYZ_CONFIG_SCHEMA + "\";" +
          "CREATE TABLE IF NOT EXISTS  " + XYZ_CONFIG_SCHEMA + "." + XYZ_CONFIG_STORAGE_TABLE
          + " (id VARCHAR(50) primary key, owner VARCHAR (50), config JSONB);" +
          "CREATE TABLE IF NOT EXISTS  " + XYZ_CONFIG_SCHEMA + "." + XYZ_CONFIG_SPACE_TABLE
  ...
  public static String createTxnPubTableSQL =
      "CREATE TABLE IF NOT EXISTS " + XYZ_CONFIG_SCHEMA + "." + XYZ_CONFIG_TXN_PUB_TABLE +
          "( " +
          "  subscription_id text NOT NULL, " +
          "  last_txn_id int8 NOT NULL, " +
          "  updated_at TIMESTAMPTZ NOT NULL DEFAULT now() " +
          "); " +
          "CREATE UNIQUE INDEX IF NOT EXISTS \"idx_xyz_txn_pub_id\" ON " + XYZ_CONFIG_SCHEMA + "." + XYZ_CONFIG_TXN_PUB_TABLE
