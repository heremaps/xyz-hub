...
- **EventHandler**: Some code statically linked into the Naksha-Hub, referred to via the class-name. 
- **Connector**: A configuration of a class implements the IEventHandler interface. An event handler normally implements some business logic. Setting the “extension” property to an extension identifier signals, that the code must be executed in an isolated process. This causes a proxy class to be instantiated, which will forward the event to the isolated process.
- **ConnectorComponent**: A component processing events via an event-pipeline using connectors.
- **Space**: A component with some configuration and a public name. It receives an event from a REST API, sending this event through the attached event-pipeline. A space can be used for any purpose, mostly used to store and modify features in a storage.
