...
class CurrentTcpConnections extends AttributedMetricCollector<Collection<Double>> {

  CurrentTcpConnections() {
    super(CurrentTcpConnections.class.getSimpleName(), COUNT);
  }

  @Override
  protected Map<Collection<Attribute>, Collection<Double>> gatherValues() {
    return HubTCPMetrics.currentConnections.entrySet().stream().collect(Collectors.toMap(
