        ...
        try {
          ...
          router.route("/hub/static/*")
              .handler(new DelegatingHandler<>(StaticHandler.create().setIndexPage("index.html"), context -> context.addHeadersEndHandler(v -> {
                //This handler implements a workaround for a bug in the StaticHandler implementation with redirection to index-pages.
                //See: https://github.com/vert-x3/vertx-web/issues/1896
                MultiMap headers = context.response().headers();
                if (headers.contains(LOCATION)) {
                  String headerValue = headers.get(LOCATION);
                  if (headerValue.endsWith("index.html/"))
                    headers.set(LOCATION, headerValue.substring(0, headerValue.length() - 1));
                }
              }), null))
