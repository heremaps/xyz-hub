...
class ResetHttpRequests extends AttributedMetricCollector<Collection<Double>> {

  ResetHttpRequests() {
    super(ResetHttpRequests.class.getSimpleName(), COUNT);
  }

  @Override
  protected Map<Collection<Attribute>, Collection<Double>> gatherValues() {
    return HubClientMetrics.resetHttpRequests.entrySet().stream().collect(Collectors.toMap(
        e -> Collections.singleton(new Attribute<>(TARGET, e.getKey())),
        e -> {
          long resetRequests = e.getValue().longValue();
          e.getValue().add(-resetRequests);
          return Collections.singleton((double) resetRequests);
        }
    ));
  }
