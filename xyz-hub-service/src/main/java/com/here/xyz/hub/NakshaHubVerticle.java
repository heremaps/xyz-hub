...
public class NakshaHubVerticle extends AbstractVerticle {
  ...
  private void onResourceRequest(@NotNull RoutingContext routingContext) {
    ...
    try {
      ...
      sendErrorResponse(routingContext, new XyzErrorException(XyzError.TOO_MANY_REQUESTS, "Failed to load resource, no more worker"));
    }
  }
  ...
  protected void notFoundHandler(final @NotNull RoutingContext routingContext) {
    routingContext.response().setStatusCode(NOT_FOUND.code()).setStatusMessage(NOT_FOUND.reasonPhrase()).end();
  }
  ...
  private void onRequestCancelled(@NotNull RoutingContext routingContext) {
    ...
    if (task != null) {
      if (task.cancel()) {
        routingContextLogger(routingContext).info("Successfully cancelled task for client side closed connection");
      } else {
