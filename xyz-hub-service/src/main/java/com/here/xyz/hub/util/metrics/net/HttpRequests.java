...
class HttpRequests extends AttributedMetricCollector<Collection<Double>> {

  HttpRequests() {
    super(HttpRequests.class.getSimpleName(), COUNT);
  }

  @Override
  protected Map<Collection<Attribute>, Collection<Double>> gatherValues() {
    return HubClientMetrics.httpRequests.entrySet().stream().collect(Collectors.toMap(
        e -> Collections.singleton(new Attribute<>(TARGET, e.getKey())),
        e -> {
          long httpRequests = e.getValue().longValue();
          e.getValue().add(-httpRequests);
          return Collections.singleton((double) httpRequests);
        }
    ));
  }
