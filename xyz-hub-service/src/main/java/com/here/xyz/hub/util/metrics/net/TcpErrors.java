...
class TcpErrors extends AttributedMetricCollector<Collection<Double>> {

  TcpErrors() {
    super(TcpErrors.class.getSimpleName(), COUNT);
  }

  @Override
  protected Map<Collection<Attribute>, Collection<Double>> gatherValues() {
    return HubTCPMetrics.exceptionCount.entrySet().stream().collect(Collectors.toMap(
        e -> Collections.singleton(new Attribute<>(TARGET, e.getKey())),
        e -> {
          long errorCount = e.getValue().longValue();
          e.getValue().add(-errorCount);
          return Collections.singleton((double) errorCount);
        }
    ));
  }
