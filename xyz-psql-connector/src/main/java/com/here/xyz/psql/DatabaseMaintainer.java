  ...
  private synchronized void initialDBSetup(String streamId, boolean autoIndexing, boolean hasPropertySearch) {
    ...
    try (final Connection connection = dataSource.getConnection()) {
      ...
      try (Statement stmt = connection.createStatement();
          ResultSet rs = stmt
              .executeQuery(MaintenanceSQL.generateCheckExtensionsSQL(hasPropertySearch, config.user(), config.database()))) {
        ...
        if (rs.next()) {
          ...
          if (!rs.getBoolean("all_ext_av")) {
            ...
            if (userHasCreatePermissions) {
              ...
              logger.error(
                  "{} - User permissions missing! Not able to create missing Extensions on database: {}@{}. Installed Extension are: {}",
      ...
      try (
          ...
          ResultSet rs = stmt.executeQuery(MaintenanceSQL.generateCheckSchemasAndIdxTableSQL(config.schema()))) {
        if (rs.next()) {
          ...
          try {
            ...
            if (!configSchema && hasPropertySearch) {
              logger.info("{} - Create missing Schema {} on {}!", streamId, MaintenanceSQL.XYZ_CONFIG_SCHEMA,
                  config.database());
      ...
      try (Statement stmt = connection.createStatement();
          ResultSet rs = stmt.executeQuery(MaintenanceSQL.generateEnsureExtVersionSQL(config.schema()))) {
        if (rs.next()) {
          /** If xyz_ext_version exists, use it to evaluate if the functions needs to get updated*/
          try (ResultSet rsVer = stmt.executeQuery(MaintenanceSQL.generateEnsureCorrectVersionSQL(config.schema()))) {
            if (rsVer.next()) {
              functionsUpToDate = (rsVer.getInt(1) == DatabaseMaintainer.XYZ_EXT_VERSION);
            }
          }
        }
        ...
        if (!functionsUpToDate) {
          ...
          try (Statement stmtSP = connection.createStatement()) {
            stmtSP.execute(MaintenanceSQL.generateSearchPathSQL(config.schema()));
            stmtSP.execute(content);
          }
    ...
    /** Check if all required H3 related stuff is present */
      if (userHasCreatePermissions) {
          ...
          logger
              .warn("{} - User permissions missing! Can not update/install H3 related functions on database': {}@{} ", streamId,
                  config.user(),
                  config.database());
      }
  }
  ...
  private synchronized void triggerIndexing(String streamId, boolean autoIndexing) {
    ...
    try (final Connection connection = dataSource.getConnection()) {
      ...
      if (progress == 0) {
        ...
        /** Maintain INDICES */
        stmt.execute(
            MaintenanceSQL.generateIDXSQL(config.schema(), config.user(), config.password(), config.database(), config.port(), mode));
      }
    } catch (Exception e) {
