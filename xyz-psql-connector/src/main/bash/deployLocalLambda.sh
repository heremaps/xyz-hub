...
jarName=${2:-"xyz-psql-connector-$(mvn -q -Dexec.executable=echo -Dexec.args='${project.version}' --non-recursive exec:exec -f ../../../pom.xml)"}
handler=${3:-'com.here.xyz.psql.PSQLXyzConnector::handleRequest'}
...
aws --endpoint "$LOCAL_STACK_HOST" lambda delete-function \
  --region us-east-1 \
  --function-name xyz-psql-connector \
  > /dev/null 2>&1
