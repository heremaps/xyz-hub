...
public class PSQLSearchIT extends PSQLAbstractIT {
  ...
  protected void addTagsToSearchObject(Map<String, Object> json, String... tags) {
    ...
    else if (json.get("propertiesQuery") instanceof List) {
      List propertiesQueryList =  (List)json.get("propertiesQuery");
      if (propertiesQueryList.size() == 0)
        json.put("propertiesQuery", List.of(List.of(tagsSearchPredicate)));
      else if (propertiesQueryList.get(0) instanceof List){
        List propertiesQueryList2 =  (List)propertiesQueryList.get(0);
        json.put("propertiesQuery", List.of(new ArrayList() {{
          add(tagsSearchPredicate);
          addAll(propertiesQueryList2);
        }}));
      }
    }
  }
