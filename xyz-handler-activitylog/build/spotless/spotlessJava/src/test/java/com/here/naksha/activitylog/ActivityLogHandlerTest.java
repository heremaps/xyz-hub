...
class ActivityLogHandlerTest {
  ...
  void test_fromActivityLog() throws IOException {
    final Feature feature =
        JsonSerializable.deserialize(
            IoHelp.openResource("activity_log_feature.json"), Feature.class);
  ...
  void test_fromActivityLogPartial() throws IOException {
    final Feature feature =
        JsonSerializable.deserialize(
            IoHelp.openResource("activity_log_partial.json"), Feature.class);
    assertNotNull(feature);
    assertNotNull(feature.getProperties());
    assertNotNull(feature.getProperties().getXyzNamespace());
    assertNotNull(feature.getProperties().getXyzActivityLog());
  ...
  void test_connectToDb() throws Exception {
    final PsqlConfig remoteActivityLogDbConfig =
        new PsqlConfigBuilder()
            .withAppName("Naksha-Psql-Test")
            .parseUrl(System.getenv("REMOTE_ACTIVITY_LOG_DB"))
            .build();
    final PsqlConfig localActivityLogDbConfig =
        new PsqlConfigBuilder()
            .withAppName("Naksha-Psql-Test")
    ...
    ActivityLogDBWriter.fromActicityLogDBToFeature(
        dataSourceLocalhost, dataSourceActivityLog, "RnxiONGZ", 10);
  }
  ...
  void test_deserialization() throws IOException {
    final Feature feature =
        JsonSerializable.deserialize(IoHelp.openResource("naksha_feature_1.json"), Feature.class);
    final Typed raw =
        JsonSerializable.deserialize(IoHelp.openResource("naksha_feature_1.json"), Typed.class);
    final JsonNode raw1 =
  ...
  void test_sqlQueryBuilder() throws IOException {
    ...
    String query =
        ActivityLogDBWriter.sqlQueryInsertConvertedFeatures(
            listFeatures, "activity", listGeos, listi);
    assertEquals(
        query,
