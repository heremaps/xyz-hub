...
public class ActivityLogDBWriter {
  ...
  public static void fromActicityLogDBToFeature(
      PsqlDataSource dataSourceLocalHost,
      PsqlDataSource dataSourceActivityLog,
      String tableName,
      Integer limit) {
    ...
    try (Connection conn = dataSourceActivityLog.getConnection()) {
      ...
      try (Connection connLocalHost = dataSourceLocalHost.getConnection()) {
        ...
        while (batchNumber < (intMaxIActivityTable + limit)) {
          String SQLSelectActiLog =
              sqlQuerySelectFromActivityLog(schema, tableName, batchNumber, limit);
  ...
  public static String sqlQuerySelectFromActivityLog(
      String schema, String tableName, int batchNumber, int limit) {
    String SQLSelectActiLog =
        "SELECT jsondata,geo,i FROM "
            + schema
            + ".\""
            + tableName
            + "\" Where i>"
  ...
  public static String sqlQuerySelectMaxIFeatures() {
    String SQLMaxIFeaturesTable =
        "select i from activity.\"Features_Original_Format\" order by i desc limit 1;";
    return SQLMaxIFeaturesTable;
  }

  public static String sqlQueryMaxIActivity(String schema, String tableName) {
    String SQLMaxIActivityTable =
        "select i from " + schema + ".\"" + tableName + "\"" + "order by i desc limit 1;";
  ...
  public static String sqlQueryInsertConvertedFeatures(
    ...
    String firstPart =
        "INSERT INTO " + schema + ".\"Features_Original_Format\"(jsondata,geo,i) VALUES ";
