...
public class ExportToFiles implements JobCompilationInterceptor {
  ...
  //TODO: That method is never called, still necessary to keep?
  static boolean canExportFromDb(Space source) throws ValidationException {
    String spaceId = source.getId();
    SpaceContext sourceContext;
    StatisticsResponse sourceStatistics;

    try {
      HubWebClient hubWebClient = HubWebClient.getInstance(Config.instance.HUB_ENDPOINT);
