...
public class StateMachineExecutor extends JobExecutor {
  ...
  private StateMachine compileToStateMachine(Job job, StepGraph stepGraph) {
    ...
    if (stepGraph.isParallel()) {
      ...
      NamedState parallelState = firstState = lastState = compileToParallelState(job, stepGraph, null);
      ...
      final List<NamedState> sequentialStates = compileExecutions(job, stepGraph.getExecutions(), null);
   ...
   * @return
   */
  private NamedState<ParallelState.Builder> compileToParallelState(Job job, StepGraph stepGraph, State.Builder previousState) {
    if (!stepGraph.isParallel())
  ...
  private Branch.Builder compileToBranch(Job job, List<StepExecution> executions) {
    ...
    final List<NamedState> sequentialStates = compileExecutions(job, executions, null);
   ...
   * @return
   */
  private List<NamedState> compileExecutions(Job job, List<StepExecution> executions, State.Builder previousState) {
    ...
    for (StepExecution execution : executions) {
      if (execution instanceof StepGraph sg) {
        if (sg.isParallel())
          ...
          states.add(compileToParallelState(job, sg, previousState));
        else
          //It's a sequential subgraph so compile all elements to states and add them
          states.addAll(compileExecutions(job, sg.getExecutions(), previousState));
      }
      else
        //It's a step, compile it into a state & add it to the map
        states.add(compile(job, (Step<?>) execution, previousState));
  ...
  private NamedState<TaskState.Builder> compile(Job job, Step<?> step, State.Builder previousState) {
  ...
  private void compile(Job job, LambdaBasedStep<?> lambdaStep, NamedState<TaskState.Builder> state) {
    ...
    ARN lambdaArn = Config.instance.STEP_LAMBDA_ARN;
    LambdaStepRequest payload = new LambdaStepRequest()
        .withType(START_EXECUTION)
        .withStep(lambdaStep);

    //TODO: This is a workaround for an open issue with AWS SDK2 for StepFunctions
    lambdaTaskParametersLookup.put(state.stateName, new LambdaTaskParameters(lambdaArn.toString(), payload.serialize()));
