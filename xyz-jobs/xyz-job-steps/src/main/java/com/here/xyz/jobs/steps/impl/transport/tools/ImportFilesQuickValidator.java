...
public class ImportFilesQuickValidator {
  ...
  public static void validate(S3DataFile s3File, EntityPerLine entityPerLine) throws ValidationException {
    try {
      validateFirstCSVLine(s3File, entityPerLine);
    }
    catch (IOException e) {
  ...
  private static void validateFirstCSVLine(S3DataFile s3File, EntityPerLine entityPerLine)
      throws IOException, ValidationException {

    logger.info("Validating first line of file {} ", s3File.getS3Key());
    ...
    try (BufferedReader reader = new BufferedReader(new InputStreamReader(countingStream, StandardCharsets.UTF_8))) {
      ...
      while ((ch = reader.read()) != -1) {
        ...
        if (ch == '\n' || ch == '\r') {
          ImportFilesQuickValidator.validateCSVLine(line.toString(), entityPerLine);
  ...
  private static void validateCSVLine(String csvLine, EntityPerLine entityPerLine) throws ValidationException {
